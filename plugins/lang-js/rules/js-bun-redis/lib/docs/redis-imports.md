...
resource: plugins/lang-js/rules/js-bun-redis/lib/redis-imports.mjs
docgen:
  crc: e150f3a1
---
...
Сканер є **частиною правила** `js-bun-redis.mdc` у системі правил `n-rules` (директорія `npm/rules/js-bun-redis/`) і використовується чек-скриптами правила для пошуку порушень у пакетах монорепо.
