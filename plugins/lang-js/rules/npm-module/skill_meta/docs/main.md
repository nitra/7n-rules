...
type: JS Module
title: main.mjs
resource: plugins/lang-js/rules/npm-module/skill_meta/main.mjs
docgen:
  crc: 1bfaac0d
...
- **Перевірки правил (programmatic)**: `npx @7n/rules fix`
...
`n-changelog.mdc` (alwaysApply) релевантне після **будь-якої** зміни файлів, не лише для релізу. Перед фінальною відповіддю виконай `npx @7n/rules fix changelog` (exit `0`) і познач результат рядком `Changelog: …` у відповіді.
