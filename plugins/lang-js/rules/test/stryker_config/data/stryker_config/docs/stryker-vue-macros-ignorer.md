...
Реєстрація `strykerPlugins` віддає Stryker один `Ignore`-плагін з іменем `vue-macros`, а `shouldIgnore` є спільним правилом відсікання для цього плагіна: він пропускає лише виклики Vue `<script setup>`-макросів із переліку `defineProps`, `defineEmits`, `defineModel`, `defineSlots`, `defineExpose`, `defineOptions`, щоб Stryker не мутав аргументи, які `@vue/compiler-sfc` має бачити статично. Якщо вузол не є викликом, якщо викликається не ідентифікатор, або якщо ім’я не належить до цього набору, мутація не блокується. Відповідь `shouldIgnore` є сигналом для Stryker: непорожнє повідомлення означає пропуск піддерева, а відсутність відповіді — продовження мутації без спеціального винятку.
