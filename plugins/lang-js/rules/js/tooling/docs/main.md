...
- `verifyOxlintRcAgainstCanonical(cfg, canonical)` — звіряє `.oxlintrc.json`
  ...
  локальні ключі дозволені), `ignorePatterns` і `jsPlugins` канону мають бути
  присутні (локальні розширення та wrapper-и дозволені), решта полів — точний
  глибокий збіг.
- `planOxlintrcFix(actual, canonical)` — чиста функція, що дзеркалить
  ...
  project-specific розширень: зайві `rules`-ключі, `ignorePatterns` і
  `jsPlugins` зберігаються, а канонічні `rules`-значення й інші поля верхнього рівня
