...
export function buildDependencyPrompt({ manifest, pkg, from, to }) {
  return [
    ...
    '5. Розрізняй два випадки. Нетривіальна або неоднозначна міграція — не вгадуй: залиш TODO-коментар у коді з посиланням на CHANGELOG, але НЕ додавай taze-виключення.',
    `   Якщо ж major-оновлення точно неможливе зараз через підтверджену зовнішню перешкоду (наприклад, залежний пакет із несумісним \`peerDependencies\`), поверни \`${pkg}\` у package.json до \`${from}\`, запусти \`bun install\` і додай постійне виключення в кореневий \`taze.config.ts\`. Спершу прочитай наявний конфіг: збережи його стиль і всі наявні налаштування. У нативному \`packageMode\` має бути рівно один запис \`'${pkg}': 'ignore'\`; не дублюй його на повторному прогоні й не створюй конфліктного другого конфіга. Без наявного конфіга створи валідний нативний конфіг taze. Безпосередньо над записом залиш коментар із причиною та перевірюваним посиланням на перешкоду: пакет і версія, несумісний peer-діапазон, URL його package.json/README або upstream issue.`,
    '',
