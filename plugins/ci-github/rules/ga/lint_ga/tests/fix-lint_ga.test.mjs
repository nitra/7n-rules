 ...
 * `fix-lint_ga.mjs` резолвить правильний target/snippet і ідемпотентний на
 * канонічному вмісті (детальні merge-кейси — у
...
import { patterns } from '../fix-lint_ga.mjs'

const CONCERN_DIR = dirname(dirname(fileURLToPath(import.meta.url)))
const TARGET_PATH = '.github/workflows/lint-ga.yml'
const SNIPPET_PATH = join(CONCERN_DIR, 'template', 'lint-ga.yml.snippet.yml')

test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'lint-ga-yml-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'ga-lint_ga-template')
    const violations = [
      { ruleId: 'ga', concernId: 'lint_ga', reason: 'policy-deny', message: 'x', file: TARGET_PATH }
    ]
...
test('файл відсутній → створюється зі snippet', async () => {
  ...
  try {
    const p = patterns.find(x => x.id === 'ga-lint_ga-template')
    const violations = [
      { ruleId: 'ga', concernId: 'lint_ga', reason: 'policy-file-missing', message: 'x', file: TARGET_PATH }
    ]
    const res = await p.apply(violations, { cwd: dir, concernDir: CONCERN_DIR })
