 ...
 * `fix-git_ai.mjs` резолвить правильний target/snippet і ідемпотентний на
...
import { patterns } from '../fix-git_ai.mjs'

const CONCERN_DIR = dirname(dirname(fileURLToPath(import.meta.url)))
const TARGET_PATH = '.github/workflows/git-ai.yml'
const SNIPPET_PATH = join(CONCERN_DIR, 'template', 'git-ai.yml.snippet.yml')

test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'git-ai-yml-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'ga-git_ai-template')
    const violations = [
      { ruleId: 'ga', concernId: 'git_ai', reason: 'policy-deny', message: 'x', file: TARGET_PATH }
    ]
...
test('файл відсутній → створюється зі snippet', async () => {
  ...
  try {
    const p = patterns.find(x => x.id === 'ga-git_ai-template')
    const violations = [
      { ruleId: 'ga', concernId: 'git_ai', reason: 'policy-file-missing', message: 'x', file: TARGET_PATH }
    ]
    const res = await p.apply(violations, { cwd: dir, concernDir: CONCERN_DIR })
