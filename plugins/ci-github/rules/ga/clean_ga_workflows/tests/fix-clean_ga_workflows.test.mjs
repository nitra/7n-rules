 ...
 * `fix-clean_ga_workflows.mjs` резолвить правильний target/snippet і
...
import { patterns } from '../fix-clean_ga_workflows.mjs'

const CONCERN_DIR = dirname(dirname(fileURLToPath(import.meta.url)))
const TARGET_PATH = '.github/workflows/clean-ga-workflows.yml'
const SNIPPET_PATH = join(CONCERN_DIR, 'template', 'clean-ga-workflows.yml.snippet.yml')

test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'clean-ga-workflows-yml-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'ga-clean_ga_workflows-template')
    const violations = [
      { ruleId: 'ga', concernId: 'clean_ga_workflows', reason: 'policy-deny', message: 'x', file: TARGET_PATH }
    ]
...
test('файл відсутній → створюється зі snippet', async () => {
  ...
  try {
    const p = patterns.find(x => x.id === 'ga-clean_ga_workflows-template')
    const violations = [
      { ruleId: 'ga', concernId: 'clean_ga_workflows', reason: 'policy-file-missing', message: 'x', file: TARGET_PATH }
    ]
    const res = await p.apply(violations, { cwd: dir, concernDir: CONCERN_DIR })
