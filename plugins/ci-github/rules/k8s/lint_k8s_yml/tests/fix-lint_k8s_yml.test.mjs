 ...
 * `fix-lint_k8s_yml.mjs` резолвить правильний target/snippet і ідемпотентний
 * на канонічному вмісті (детальні merge-кейси — у
...
import { patterns } from '../fix-lint_k8s_yml.mjs'

const CONCERN_DIR = dirname(dirname(fileURLToPath(import.meta.url)))
const TARGET_PATH = '.github/workflows/lint-k8s.yml'
const SNIPPET_PATH = join(CONCERN_DIR, 'template', 'lint-k8s.yml.snippet.yml')

test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'lint-k8s-yml-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'k8s-lint_k8s_yml-template')
    const violations = [
      { ruleId: 'k8s', concernId: 'lint_k8s_yml', reason: 'policy-deny', message: 'x', file: TARGET_PATH }
    ]
...
test('файл відсутній → створюється зі snippet', async () => {
  ...
  try {
    const p = patterns.find(x => x.id === 'k8s-lint_k8s_yml-template')
    const violations = [
      { ruleId: 'k8s', concernId: 'lint_k8s_yml', reason: 'policy-file-missing', message: 'x', file: TARGET_PATH }
    ]
    const res = await p.apply(violations, { cwd: dir, concernDir: CONCERN_DIR })
