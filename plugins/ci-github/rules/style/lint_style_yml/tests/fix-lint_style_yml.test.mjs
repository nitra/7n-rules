 ...
 * `fix-lint_style_yml.mjs` резолвить правильний target/snippet і
...
import { patterns } from '../fix-lint_style_yml.mjs'

const CONCERN_DIR = dirname(dirname(fileURLToPath(import.meta.url)))
const TARGET_PATH = '.github/workflows/lint-style.yml'
const SNIPPET_PATH = join(CONCERN_DIR, 'template', 'lint-style.yml.snippet.yml')

test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'lint-style-yml-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'style-lint_style_yml-template')
    const violations = [
      { ruleId: 'style', concernId: 'lint_style_yml', reason: 'policy-deny', message: 'x', file: TARGET_PATH }
    ]
...
test('файл відсутній → створюється зі snippet', async () => {
  ...
  try {
    const p = patterns.find(x => x.id === 'style-lint_style_yml-template')
    const violations = [
      { ruleId: 'style', concernId: 'lint_style_yml', reason: 'policy-file-missing', message: 'x', file: TARGET_PATH }
    ]
    const res = await p.apply(violations, { cwd: dir, concernDir: CONCERN_DIR })
