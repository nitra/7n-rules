 ...
 * `fix-clean_merged_ignore_branches.mjs` резолвить правильний
 * target/snippet і ідемпотентний на канонічному вмісті (детальні
 * merge-кейси — у `scripts/lib/tests/template-deep-merge.test.mjs`).
 */
...
import { patterns } from '../fix-clean_merged_ignore_branches.mjs'
...
test('канонічний вміст → idempotent (touchedFiles порожній)', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'clean-merged-ignore-branches-'))
  try {
    mkdirSync(join(dir, '.github/workflows'), { recursive: true })
    writeFileSync(join(dir, TARGET_PATH), readFileSync(SNIPPET_PATH, 'utf8'), 'utf8')
    const p = patterns.find(x => x.id === 'abie-clean_merged_ignore_branches-template')
    const violations = [
      {
        ruleId: 'abie',
