...
`buildUvDependencyPrompt` формує інструкцію для LLM лише після того, як bump уже виконано детерміновано: промпт веде від звірки breaking changes на сторінці пакета в https://pypi.org/project/ до пошуку використань у коді, а далі — або до відсутності змін, або до точкового рефакторингу й запуску наявних перевірок. Посилання на релізні нотатки пакета йде через сторінку PyPI, щоб мати один стабільний вхід до історії версій.
