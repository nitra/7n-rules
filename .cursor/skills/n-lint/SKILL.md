...
**Черга вбудована в CLI.** `npx @nitra/cursor lint` тримає **глобальний machine-wide лок** (spec 2026-07-03): у кожен момент виконується один lint-процес на машину, паралельні запуски самі стають у чергу (`⏳ lint-global: чекаю, лок тримає pid …`) і стартують після звільнення. Ідентичний повторний запуск на незміненому дереві дедуплікується (`♻️ … пропускаю`). Агенту нічого координувати вручну — просто запускай команду.

### Що робити агенту під час виконання цього скілу

1. **Дельта-лінт** (`npx @nitra/cursor lint`) — можна запускати повторно; одночасні запуски (свої чи чужих субагентів) серіалізуються самі.
2. **`lint --full`** у цьому скілі **не запускати** — це CI-команда, не задачна.
3. Якщо запуск висить із `⏳ lint-global: чекаю…` — інший lint ще працює; це штатна черга, не зависання. Fail-closed таймаут черги — 20 хв.
