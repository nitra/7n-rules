...
5. **Результат** — коротко опиши, що саме виправлено; якщо щось блокує нульовий exit code — залиш чітке пояснення й наступні кроки для людини. Канонічний `lint` у root `package.json` — `n-cursor lint`: оркестратор послідовно прогонить `lint-ga`, `lint-js`, `lint-rego`, `lint-style`, `lint-text`, `lint-security`, `oxfmt` (fail-fast) і друкує наприкінці таблицю `⏱ Lint timing` з часом кожного кроку. **Скопіюй цю таблицю у фінальне резюме** (як код-блок).
