...
## Підсумок у відповіді користувачу

Наприкінці виводу `npx @nitra/cursor fix` пакет друкує таблицю тайменгів `⏱ Fix timing` з часом виконання кожного `fix-<rule>`. **Скопіюй цю таблицю у фінальне резюме** (як код-блок), щоб користувач бачив, скільки часу зайняло кожне правило і де bottleneck.
