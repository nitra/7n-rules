...
Драфт уточнює файловий контракт `n-cursor graph`:

- режим виконавця кодується sentinel-файлами `a.md` / `h.md`, а `task.md` залишається стабільним;
- відсутність `a.md` і `h.md` означає `unassigned`;
- `running_<pid>_until_<ts>` поєднує PID і deadline для cleanup через `kill -0 <pid>`;
- composite-вузли отримують явний `fact_NNN.md`, а не окремий `composite_NNN.md`;
- `deps/` дзеркалить структуру `tasks/` для крос-рівневих залежностей;
- `graph kill` завжди cascade і створює checkpoint commit, undo виконується через `git revert`.

Transcript не містить підтверджених негативних наслідків для більшості пунктів, окрім вимоги clean working tree для `graph kill`.

## Update 2026-06-07

Драфт доповнює дизайн `n-cursor graph` такими деталями:

- фінальна таблиця станів розділяє `waiting-plan` і `waiting-run`, де стан відповідає на питання «що потрібно», а `a.md`/`h.md` — «хто виконує»;
- `running_<pid>_until_<ts>` підтримує cleanup при старті нового `graph run` і під час `watch`, а `grace_period` у `.n-cursor.json` враховує clock skew;
- `fact_NNN.md` для composite пишеться wrapper-ом після merge останньої дитини та може рекурсивно закривати батьків;
- `audit-result_NNN.md` позначено як deletable для повторного аудиту того самого `fact_NNN.md`;
- для compact prior attempts наступному агенту передаються секції `## Completed`, `## Blockers`, `## Next Attempt`, а не всі повні `run_NNN.md`.

Окремо згадано hash-based differential cascade через `hash: sha256:<hash секції ## Result>` у `fact_NNN.md`, але transcript не містить підтверджених негативних наслідків цього рішення.

## Update 2026-06-07

Зафіксовано наскрізний файловий контракт `n-cursor graph` зі строгим listing-інваріантом: стан вузла визначається переліком файлів і директорій без читання вмісту.

Додаткові деталі з драфта:

- фінальні стани: `unassigned`, `waiting-plan`, `blocked`, `waiting-run`, `running`, `stalled`, `failed`, `pending-audit`, `resolved`;
- пріоритет станів: `resolved` > `pending-audit` > `stalled` > `running` > `failed` > `blocked` > `waiting-run` > `waiting-plan` > `unassigned`;
- `running_0_until_0` використовується для людського виконання;
- `graph migrate` виконується при кожному релізі, тому `schema_version:` не потрібен;
- `mode:` видалено з `plan_001.md`, єдине джерело правди — `a.md`/`h.md`;
- `allow_replan: true` в `a.md` дозволяє агенту автономно перемикати `hint: atomic` у composite;
- `agent_concurrency` обмежує чергу агентів, людські worktrees не обмежуються;
- `NCURSOR_TASKS_DIR` вказує активний `tasks/` root, один `n-cursor watch` працює на один root;
- `plan_001.md` завжди один, `graph replan` архівує попередній план до `history/<ts>-replan/`.

Transcript зазначає, що ризик clock skew на distributed FS лишається out of scope для MVP.
