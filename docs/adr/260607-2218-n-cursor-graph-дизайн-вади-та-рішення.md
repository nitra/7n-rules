...
Уточнено пакет рішень для `n-cursor graph` зі специфікації `docs/думка.MD`:

- `running_<pid>_until_<ts>` замінює `running_until_<ts>`; cleanup виконується при старті `graph run` і при кожному скані watch через `kill -0 <pid>` без читання вмісту sentinel.
- `waiting-plan` / `waiting-run` замінюють `waiting`, `human-pending`, `needs-plan`; стан відповідає на питання “що потрібно”, а `a.md`/`h.md` — “хто виконує”.
- `deps/` може бути вкладеною структурою, що дзеркалює `tasks/`, наприклад `deps/research/analyze.md` → `tasks/research/analyze/`.
- Composite-вузли отримують явний `fact_NNN.md`, який пише `graph done` wrapper після merge останньої дитини; `NNN = count(існуючих fact_*.md) + 1`.
- Зафіксований edge case: `budget_hard_sec: 0` не має створювати sentinel із deadline у минулому; transcript вказує ризик миттєвого `stalled`, але не містить повної специфікації обробки.

## Update 2026-06-07

Уточнено стандарт іменування файлів у `deps/`:

- Усі файли залежностей мають розширення `.md`.
- `graph scan` читає шлях через `ls -R deps/`, після чого обрізає `.md` для отримання dep-id.
- Для вкладених залежностей приклад алгоритму: `deps/research/analyze.md` → `research/analyze` → `tasks/research/analyze/fact_*.md`.
- Рішення усуває суперечність у специфікації `docs/думка.MD`, де частина прикладів показувала `<dep-node-id>` без розширення, а частина — `<dep-node-id>.md`.

## Update 2026-06-07

Додатково зафіксовано рішення з design review `n-cursor graph`:

- `audit-result_NNN.md` є deletable: `graph audit-retry <path>` видаляє result, а watch повторно запускає аудит того самого `pending-audit_NNN.md` / `fact_NNN.md`; `pending-audit_NNN.md` і `fact_NNN.md` залишаються immutable.
- При breaking change схеми обрано примусову міграцію через `graph migrate`; transcript формулює принцип “версія схеми = версія інструменту `n-cursor`”, без `schema_version:` у кожному файлі.
- Для composite-вузлів `fact_NNN.md` містить агрегований `## Summary` із summary дітей.
- При інвалідації дитини потрібен cascade: батько отримує `invalidated`, а після повторного resolve створюється новий `fact_NNN.md`.

## Update 2026-06-07

Уточнено додаткові рішення для `docs/думка.MD`:

- `mode:` видаляється з frontmatter `plan_NNN.md`; актуальний mode визначається лише через `a.md` / `h.md`.
- Для failed runs агент має отримувати компактне summary попередніх провалів замість повних `run_NNN.md`; джерело summary — секції `## Blockers` і `## Next Attempt`.
- `## Blockers` і `## Next Attempt` стають обовʼязковими для `run_NNN.md` з `result: failed`.
- `audit-result_NNN.md` можна видаляти лише через retry аудиту; це виняток із загальної immutability artifact-файлів.

## Update 2026-06-07

Додано уточнення до graph design:

- Список mutable/deletable файлів включає `a.md`, `h.md`, `invalidated`, `running_<pid>_until_<ts>` і deletable-only `audit-result_NNN.md`.
- Для retry аудиту `graph audit-retry <path>` видаляє `audit-result_NNN.md`, де `NNN` відповідає pending audit без result.
- При зміні схеми CLI має guard: якщо виявлено файли старої схеми, потрібно вимагати `graph migrate` перед продовженням.
- Composite planning має створювати `a.md` або `h.md` для дочірніх вузлів; інакше діти залишаються `unassigned`, і runner їх пропускає.

## Update 2026-06-07

Зафіксовано пізніші уточнення після внесення змін у `docs/думка.MD`:

- `run-summary.md` використовується як bounded context для агента після попередніх failed спроб; transcript описує LLM-generated summary як спосіб уникнути переповнення context window.
- Версійність схеми зберігається в `.n-cursor.json` через `graph_schema`; `graph migrate` застосовує chain міграцій і оновлює значення schema.
- Differential invalidation проходить reverse dep lookup через `tasks/**/deps/` і ставить `invalidated` тільки залежним вузлам; незалежні гілки залишаються resolved.
- Watch throttling рахує активні `running_<pid>_until_*` і не spawnить нові worktrees, якщо досягнуто `max_worktrees`.
- `stale_grace_period_sec` у `.n-cursor.json` додає буфер для clock skew при stalled detection; transcript також зазначає, що `kill -0 <pid>` не застосовується між різними хостами на distributed FS.

## Update 2026-06-07

Додано фінальні уточнення design review:

- Для orphan detection обрано не ускладнювати MVP окремою `children/` директорією; orphan cleanup не автоматизується, а може бути перевірений окремим integrity/status сценарієм.
- `graph kill` трактує cascade як default для correctness; transcript також згадує escape hatch `--no-cascade`, але основний мотив — не довіряти LLM-недетермінізму при partial invalidation.
- Для агентського паралелізму розглянуто `agent_concurrency` у `.n-cursor.json`: обмеження стосується agent worktrees, а не human worktrees.
- Composite planning має призначати `a.md` / `h.md` дітям під час spawn, щоб runner не залишав їх у `unassigned`.

## Update 2026-06-07

Уточнення до файлового контракту `n-cursor graph`:

- `graph kill` має завжди виконувати cascade та checkpoint commit; undo виконується через `git revert`, а не через окрему команду reset.
- `audit-result_NNN.md` позначається як deletable для повторного аудиту того самого `fact_NNN.md`.
- `composite_NNN.md` прибрано; resolved-стан composite вузла фіксується через `fact_NNN.md`.
- TODO щодо clock skew додано до секції загроз.
- Для режиму виконавця використовуються sentinel-файли `a.md`/`h.md` поряд зі стабільним `task.md`; відсутність обох файлів означає `unassigned`.
- Стан має відповідати на питання «що потрібно зробити», а `a.md`/`h.md` — «хто це робить».
- `running_<pid>_until_<ts>` поєднує PID і deadline; cleanup виконується при старті нового `graph run` і на кожному тіку `n-cursor watch`.
- `deps/` дзеркалює структуру `tasks/`, щоб крос-рівневі залежності визначались через listing без читання вмісту.

Transcript-факти: специфікація оновлювалась у `docs/думка.MD`; для `graph kill` checkpoint commit має вигляд `git add tasks/<path>/**/invalidated && git commit -m "graph: kill tasks/<path>"`; `--only-self` виключено як такий, що може зламати консистентність графу.

## Update 2026-06-07

Додаткові уточнення до контракту `n-cursor graph`:

- `a.md` schema містить `model_tier`, `skills`; `h.md` schema містить `qualification`.
- Стани очікування розділяються на `waiting-plan` і `waiting-run`: стан описує потрібну дію, sentinel `a.md`/`h.md` описує виконавця.
- Для `running_<pid>_until_<ts>` stalled condition враховує `grace_period`; у transcript згадано `grace_period: 60` у `.n-cursor.json` для clock skew.
- `graph done` для composite після merge останньої дитини пише `fact_NNN.md` і може рекурсивно закрити ланцюг батьків.
- `audit-result_NNN.md` deletable: `graph audit-retry <path>` видаляє результат аудиту й повертає вузол у `pending-audit` без нового виконання задачі.
- Для наступних спроб агенту передається compact summary з `## Completed`, `## Blockers`, `## Next Attempt`, а не всі повні `run_NNN.md`.
- Обговорено hash-based differential cascade: `fact_NNN.md` може містити `hash: sha256:<hash секції ## Result>`, щоб при незмінному результаті не ре-ранити downstream.

Негативні наслідки для більшості цих уточнень transcript не зафіксував.

## Update 2026-06-07

Зафіксовано узагальнений файловий контракт задачного графу `n-cursor graph`:

- Обрано гібридний файловий дизайн зі строгим listing-інваріантом: стани вузла визначаються переліком файлів і директорій без читання вмісту.
- Фінальна таблиця станів включає `unassigned`, `waiting-plan`, `blocked`, `waiting-run`, `running`, `stalled`, `failed`, `pending-audit`, `resolved`.
- Пріоритет станів у transcript: `resolved` > `pending-audit` > `stalled` > `running` > `failed` > `blocked` > `waiting-run` > `waiting-plan` > `unassigned`.
- `running_0_until_0` використовується для людини; `graph done --actor human` перевіряє `## Done when` і пише `fact_NNN.md` з `actor: human`.
- Внутрішній LLM-аудитор видалено; агент self-verifies `## Done when`, а зовнішній async аудит вмикається через `require_approval: true` і файли `pending-audit_NNN.md`.
- `graph migrate` має виконуватись при кожному релізі; `schema_version:` не потрібен.
- `mode:` видалено з `plan_001.md`; джерело правди — `a.md`/`h.md`.
- `allow_replan: true` у `a.md` дозволяє агенту автономно переключити `hint: atomic` на composite; за замовчуванням агент має завершити `failed` із пропозицією для людини.
- `agent_concurrency` обмежує чергу агентів; людські worktrees без обмежень.
- Monorepo має окремий `tasks/` root для кожного workspace; `NCURSOR_TASKS_DIR` вказує активний root; один `n-cursor watch` працює на один root.

Повʼязані файли з transcript: `docs/думка.MD`, `/Users/vitaliytv/.claude/projects/-Users-vitaliytv-www-nitra-cursor/memory/project_graph_design_review.md`.
