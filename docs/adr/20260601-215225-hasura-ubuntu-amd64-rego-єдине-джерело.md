...
- Мінімальну дозволену версію `hasura/graphql-engine` у `allowed_hasura_images` піднято з `v2.49.0.ubuntu.amd64` до `v2.49.2.ubuntu.amd64`.
- Оновлено canonical-image та digest-image тест-кейси у `npm/rules/k8s/policy/manifest/manifest_test.rego` до `v2.49.2`.
- Посилання `v2.49.0` без суфікса у `npm/rules/k8s/js/tests/manifests/tests/check-schema.test.mjs` залишено, бо transcript фіксує його як generic-маркер HTTPRoute-перевірки, не частину whitelist.
