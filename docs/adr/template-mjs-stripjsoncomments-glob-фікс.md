...
Chosen option: "Regex з альтернативою для рядкових літералів", because вираз-альтернатива гарантує, що вміст у лапках пропускається нетронутим і лише справжні коментарі поза string-літералами видаляються; попередній regex без розрізнення контексту структурно не здатен правильно обробити JSON із glob-патернами в значеннях.
