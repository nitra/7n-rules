...
## More Information
Фінальна схема `task.md` включає фронтматер (`created_at`, `parent`, `deps`, `budget_sec`) і три обов'язкові секції `## Task`, `## Done when`, `## Inputs`. Файл: `docs/думка.MD`.
...
## More Information
`run_NNN.md` — immutable, пишеться після спроби. Секції: `## Reasoning` (обов'язкова), `## Script` (wrapper-скрипт, якщо впало), `## Ref` (посилання на `outputs_NNN.md` або `patches/NNN-plan.md`). `repair_context.md` прибрано; `budget_sec` перенесено в `task.md`. Файл: `docs/думка.MD`.
...
## More Information
Правило: до `git worktree add` — вільне редагування і видалення; після — тільки нові файли. Зафіксовано в розділі «Контракт» `docs/думка.MD`.
...
## More Information
Директорія `ops/` повністю видалена зі схем і файлової структури в `docs/думка.MD`.
...
## More Information
Перейменування: `операції/` → `ops/`, `патчі/` → `patches/`, `підграф/` → `subgraph/`, `вхідні.md` → `inputs.md` (потім злито в `task.md`), `вихідні.md` → `outputs_NNN.md`, `місія.md` → `task.md`. Правило зафіксовано в розділі «Контракт» `docs/думка.MD`.
...
## More Information
`deps:` містить список `id` тільки siblings (вузлів у тому самому `subgraph/`). Поле `port:` прибрано — фактичні посилання на дані описуються через `ref:` у секції `## Inputs`. Файл: `docs/думка.MD`.
