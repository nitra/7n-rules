...
### Спрощення UX до чотирьох канонічних форм (v1.13.72)

Після першої реалізації надлишкові форми видалено. Канонічні:

```bash
npx @nitra/cursor skill list
npx @nitra/cursor skill taze
npx @nitra/cursor skill cursor taze "онови залежності"
npx @nitra/cursor skill claude taze
```

Видалено: `skill prompt …`, підкоманда `claude` у `n-cursor`, bins `n-skills` (`npm/bin/n-skills.js`) та `n-claude` (`npm/bin/n-claude.js`); `"bin"` у `npm/package.json` повернуто до `{ "n-cursor": "./bin/n-cursor.js" }`.

Архітектура: `npm/scripts/skills-cli.mjs` (list, buildSkillPrompt, runSkillsCli), `npm/bin/n-cursor.js` (case 'skill'), unit-тести: `npm/scripts/skills-cli.test.mjs` (listSkillIds, normalizeSkillId, buildSkillPrompt, runSkillsCli).

## Update 2026-05-21

### Склад промпта скілу: SKILL.md + завдання + контекст CWD

Агент отримує три секції: `# Task` (текст завдання), `# Skill` (вміст `SKILL.md`), `# Current project` (директорія + `package.json`, `tsconfig.json`, `.n-cursor.json` якщо існують). Реалізовано у `buildSkillPrompt` у `npm/scripts/skills-cli.mjs`; файли читаються з `process.cwd()` на момент виклику `npx`.

### Обґрунтування: підкоманда в `@nitra/cursor`, не окремий пакет

Архітектура `@nitra/skills` з власним bin розглядалась і відхилена: скіли вже фізично в `npm/skills/<id>/SKILL.md` пакета `@nitra/cursor` — окремий пакет потребував би дублювання або cross-package залежності.
