---
type: ADR
title: "LLM-нормалізація чернеток ADR без авто-коміту"
---
...
Ручний запуск при заблокованих порогах: `ADR_NORMALIZE_THRESHOLD=0 ADR_NORMALIZE_MIN_INTERVAL_HOURS=0 bash .claude/hooks/normalize-decisions.sh`. Dry-run перед реальним прогоном: додати `ADR_NORMALIZE_DRY=1`.

Стратегія комітування: батчи по 10 чернеток (розмір за замовчуванням), коміт після кожного батчу (`git add docs/adr/ && git commit -m "adr: normalize batch"`). Такий підхід дає атомарні коміти з можливістю точкового revert.

Перший батч (із 66 чернеток): 14 файлів змінено, `+134 / −864` рядки, коміт `7c62d53`. Після першого батчу лишилося 61 чернетка.

## Update 2026-05-21

LLM-бекенд нормалізації: `claude` CLI, модель `sonnet`. Після 2 батчів залишилося 58 чернеток із 66 вихідних. Коміт-повідомлення: `adr: normalize batch`.
