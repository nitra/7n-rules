...
Підготовлено `n-llm-patch`-промпт для репозиторію `@nitra/eslint-config` з інструкцією відновити `@e18e/eslint-plugin` у `dependencies` і узгодити відсутній запис `[3.10.0]` у CHANGELOG. Після злиття upstream-фіксу явну devDependency у корені монорепо можна буде прибрати, повернувшись до транзитивного provide.
