...
### Правило `worktree` — pure-doc (J1), нормалізація сироти

`n-worktrees.mdc` існував лише в `.cursor/rules/` без канонічного джерела в `npm/rules/worktree/`, що порушувало інваріант пакету: `discoverBundledRuleNames` не знав про нього і міг видалити при наступному sync.

Обрано **J1 — pure-doc**: `npm/rules/worktree/worktree.mdc` як документація конвенції без programmatic check. CLI вже гарантує правильне розташування; окремий `check` дублював би гарантії tool. YAGNI — check структури worktree має сенс пізніше, якщо worktree почнуть створювати повз CLI.

Альтернатива **J2** (doc + programmatic check, дзеркалення `prune`-логіки) відхилена.

**Результат**: `npm/rules/worktree/worktree.mdc` є каноном; `.n-cursor.json` містить `"worktree"` у `rules`; `CLAUDE.md` оновлено (`n-worktrees.mdc` → `n-worktree.mdc`). Коміти: `f0c3e6c` (видалення сироти), `d443126` (канонічне джерело + sync).

## Update 2026-06-01

**Автоматичний вибір вільної назви у `worktree add`:** `worktree add "main-fix"` падав із `fatal: a branch named 'main-fix' already exists` при колізії (stale worktree з попереднього запуску). CLI тепер сам перевіряє зайнятість і обирає першу вільну назву.

Реалізація:
- `npm/scripts/lib/worktree.mjs` — нова чиста функція `firstFreeBranch(branch, isTaken, limit)`: повертає `base`, або `base2`, `base3`, … (перший варіант, для якого `isTaken` повертає `false`).
- `npm/scripts/worktree-cli.mjs` — `cmdAdd` викликає `firstFreeBranch` перед `git worktree add`, перевіряючи зайнятість за `git show-ref --verify` і за наявністю checkout-каталогу; виводить `ℹ️ гілка/worktree "…" уже існує — обрано "…"`.
- Нові тести: 6 unit-кейсів для `firstFreeBranch` + інтеграційний тест (зайнята назва → числовий суфікс); всього 22/22 passed.
- Change-файл: `npm/.changes/1780304440411-2b9701.md` (bump: patch, Fixed).
