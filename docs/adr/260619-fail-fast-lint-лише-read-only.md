...
- CI/deploy має запускати `npx @nitra/cursor lint --full --read-only`.
- `--read-only` означає detect-only: без мутацій і без LLM; якщо правило має `llmFix:true`, CI не викликає LLM, а падає non-zero, якщо порушення не виправлено до push.
- `--full` потрібен у CI, бо без нього конформність-фаза не виконується і структурні правила не перевіряються повністю.
- Локальна dev-команда лишається `npx @nitra/cursor lint`, де fix-mode може мутувати файли й використовувати LLM opt-in там, де це дозволено правилом.
