...
- Перенесення `coverage`-гейту на `release`-only (не в per-step `flow verify`)
...
- `base_commit` для `git diff` береться зі стану flow (`.flow.json#metadata.base_commit`)
...
- `npm/scripts/dispatcher/lib/commands.mjs:141` — `flow verify` (consumer)
...
## Update 2026-06-01 (considered options та деталі git diff)

Повний список розглянутих варіантів: scope через `--changed` (обраний); level/risk-scaled gates (coverage лише для L≥2); конфіг через `.n-cursor.json#flow.gates`; `coverage --no-mutation` (новий прапор CLI); Stryker `--incremental` (ортогональний); повне видалення coverage з `DEFAULT_GATES`.

**`git diff <base_commit>` без `..`:** порівнює base із робочим деревом — покриває committed + staged + unstaged одним викликом. `base_commit` читається з `.flow.json` (так само, як у `review.mjs:29`). Новий helper: `collectChangedFilesSince(base, cwd)` у `npm/scripts/lib/changed-files.mjs`.

**Порожній changed-scope = pass (exit 0):** root без змінених JS-файлів (doc-правки, Rust-код) → `runCoverageSteps` завершується достроково з exit 0, `COVERAGE.md` не перезаписується — пряме логічне слідство scoped-підходу. Тест-кейс: `coverage --changed`, 0 JS-файлів → exit 0, файл не створюється.

## Update 2026-06-02 (fail-closed, Rust, known bug)

`collectChangedFilesSince(base, cwd)` — fail-closed при недосяжному `base` (throws замість fallback на HEAD), бо поза flow у scoped-режимі функція не викликається.

Rust-провайдер (`npm/rules/rust/coverage/coverage.mjs`): пропускає crate, якщо немає змінених `.rs` або `Cargo.*` файлів.

**Відомий баг (зафіксований flow review L1):** у `npm/rules/js-lint/coverage/coverage.mjs` (~рядок 335) повернений exit code `runStryker` ігнорується (`await runner.runStryker(...)` без перевірки результату) — підриває контракт «змінений src без тестів → NoCoverage-мутанти → fail». На момент сесії не виправлено.

Зачеплені файли: `npm/scripts/lib/changed-files.mjs`, `npm/scripts/dispatcher/lib/reviewer.mjs`, `npm/rules/test/coverage/coverage.mjs`, `npm/rules/js-lint/coverage/coverage.mjs`, `npm/rules/rust/coverage/coverage.mjs`, `npm/bin/n-cursor.js`, `npm/rules/flow/flow.mdc`, `npm/rules/test/test.mdc`. Worktree: `.worktrees/feat-coverage-changed-gate`, гілка `feat/coverage-changed-gate`.
