...
Уточнено контракт детермінізму для нових скіл-оркестраторів:

- convergence-loop має завершуватися лише після per-rule check-gate;
- check-gate виконує `fix <rule> --json` і приймає стан тільки коли правило повертає `ok`;
- escalation ladder лишається `local → haiku → sonnet`;
- кожен tier-worker потребує власного `try/catch`, бо SDK кидає помилку при `maxTurns`, а не повертає звичайний результат;
- антипатерни: `cap` без справжньої конвергенції та самозвіт моделі «готово» без перевірки.

Додано результат tool-free експерименту: `gemma3:4b --mode text --no-tools` відхилено для config-фіксів, бо warm inference на реальних violation-промптах займав 78–90s і модель галюцинувала точні рядкові значення. Для самоописових violation output пріоритет має T0-auto: детермінований парсинг violation text і програмний patch без LLM-токенів. Приклад: regex `/recommendations має містити "([^"]+)"/` витягує extension id і оновлює `extensions.json`.

## Update 2026-06-06

- Додано проміжний рівень T0-auto між hardcoded T0 і LLM T1: violation-output `fix --json` парситься regex-ами, а виправлення застосовуються програмно без LLM-виклику.
- Підтримані патерни: `vscode-ext-add` (`recommendations має містити "X"`) і `rm-forbidden-file` (`Знайдено заборонений файл: X`).
- Реалізація: `npm/skills/fix/js/t0.mjs` (`applyT0Auto`, `filterT0AutoRules`, `runT0AutoCli`), тести — `npm/skills/fix/js/tests/t0.test.mjs`, CLI — `n-cursor fix-t0`.
- Експеримент із `gemma3:4b` показав, що для config-JSON задач локальна 4B-модель повільна й ненадійна: warm inference 78–90 секунд і галюцинації точних рядків.
- Для `gemma3:4b` у pi CLI потрібен `--no-tools`, бо модель відхиляє tools API навіть у `--mode text`; прямий тест із `--no-tools` повернув коректний JSON, але latency лишилась неприйнятною для production-фіксів.
- У `scripts.mdc` зафіксовано контракт детермінізму оркестратора: convergence-loop, check-gate, escalation ladder local→haiku→sonnet, SDK-error як tier-fail і fail-fast для локальних воркерів.
