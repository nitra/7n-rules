...
Transcript уточнив порівняння orchestrated vs one-shot для Tier 1 docgen:

- Для `gemma3:4b` orchestrated режим залишився рекомендованим, бо JS гарантує структуру, а модель лише формулює прозу.
- Секційно-мінімальний контекст v2 подає повний код лише в секцію `Поведінка`; інші секції отримують факт-лист.
- Для `gemma4:4b` transcript зафіксував несумісність із секційно-мінімальним режимом: секції без коду повертали `""`.
- Негативні маркери у fact-list (`Кешування: НЕМАЄ`, `Робота з мережею: немає`) використовуються як дешевий deterministic guard проти типових галюцинацій.

Це є уточненням до вже прийнятого рішення про orchestrated Tier 1 docgen, а не новим самостійним ADR.

## Update 2026-06-06

Уточнено v2 оркестрованого docgen-конвеєра:

- Код передається лише у секцію `Поведінка`; `Огляд`, `Публічний API` і `Гарантії` отримують тільки `factsSummary`.
- Причина: stateless `/api/chat` не дає надійного KV-cache reuse на 8 GB M2, тому повний код у кожній секції робив orchestrated режим у 2–5× повільнішим.
- Бенч показав покращення: overlay 310s→77s, k8s 141s→55s.
- Додано deterministic Stage-2 `stripSignatures`: regex-зріз `function(args)` → `function` без LLM-токенів.
- Додано негативні маркери у facts: `Кешування: НЕМАЄ — не згадуй кеш у гарантіях`, щоб зменшити cache hallucination.
- `gemma4:4b` придатна лише для one-shot: orchestrated режим повертав `""` для секцій без коду в контексті; рекомендований Tier 1 лишається `gemma3:4b` orchestrated.
