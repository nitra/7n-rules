...
`shellcheck` завалювався з ненульовим exit-кодом на SC1091 при перевірці `.claude/hooks/capture-decisions.sh` та `.claude/hooks/normalize-decisions.sh` (і їх копій у `npm/.claude-template/hooks/`). Директива `# shellcheck source=npm/.claude-template/hooks/lib/tooling-only.sh` вказувала на абсолютний відносний шлях, недоступний shellcheck без прапора `-x`. Фінальний прогін у `run-shellcheck.mjs` запускається без `-x`, тому будь-який ненульовий exit-код із shellcheck у `runFinalShellcheck` призводить до падіння `lint-text`.
...
Змінено чотири файли (однаковий однорядковий патч у кожному):
