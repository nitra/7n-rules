...
### Спільні модулі визначення (`npm/scripts/utils/`)
 ...
 * (наприклад `app/` у mail app), для single-package — корінь cwd. Використовується
...
Контракт `null` (замість throw) — щоб концерни test-правила могли gracefully skip-пропустити, а не падати при відсутності manifest.
...
**Сценарій:** mail app-подібний проєкт з `js-lint` і `rust` у `.n-cursor.json#rules`, без mutation-tooling.
...
1. **Спільні модулі визначення:** створити `npm/scripts/utils/resolve-js-root.mjs` + `resolve-cargo-manifest.mjs` з тестами в `tests/`. TDD: контракти `null`-return для missing fixtures.
