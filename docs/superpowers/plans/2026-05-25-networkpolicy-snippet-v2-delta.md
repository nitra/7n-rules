...
for (const doc of docs) {
  ...
  const annotations = /** @type {Record<string, unknown>} */ (/** @type {any} */ (doc).metadata?.annotations ?? {})
  const annotatedKind =
    typeof annotations['nitra.dev/workload-kind'] === 'string'
...
```js
const needsMigration = docs.some(d => networkPolicyHasLegacyCatchAllEgress(d) || networkPolicySpecDiffersFromCanon(d))
```
...
spec:
  ...
  ingress:
    - from:
        - podSelector: {}
  egress:
${stringify(loadSnippetSpec('deployment').egress)
  .split('\n')
  .map(l => (l ? '    ' + l : l))
  .join('\n')}
`
