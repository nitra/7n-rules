...
**Goal:** Перенести coverage-агрегатор mlmail (JS + Rust покриття + мутаційне тестування) у `@nitra/cursor` як канонічну CLI-команду `n-cursor coverage` із per-rule провайдерами, discovery через `.n-cursor.json#rules` і інтринсивним `withLock`-серіалізатором. Видалити локальні `scripts/coverage.js` + `with-lock.js` з mlmail.
...
### Видаляються в `mlmail` (PR2, після релізу `@nitra/cursor`)
...
Чисті функції з mlmail's `scripts/coverage.js` (`addCoverage`, `addMutation`, `formatCoverage`, `formatScore`, `renderMarkdown`) — переносимо в оркестратор; винесемо на початок файлу для unit-тестування.
...
describe('js-lint coverage collect()', () => {
  ...
  test('падає з explainer-ом якщо коді JS-coverage exit ≠ 0', async () => {
    const dir = makeFixture({ scripts: { 'test:coverage': 'bun test --coverage' } })
    const runner = {
      async runJsCoverage() { return 1 },
      async runStryker() { return 0 }
    }
...
const defaultRunner = {
  async runJsCoverage({ cwd, lcovDir }) {
    const proc = Bun.spawn(
      ['bun', 'run', 'test:coverage', '--coverage-reporter=lcov', `--coverage-dir=${lcovDir}`],
...
describe('rust coverage collect()', () => {
  test('парсить llvm-cov JSON + cargo-mutants outcomes.json', async () => {
    ...
    const runner = {
      ...
      async runCargoMutants({ manifestPath, outDir }) {
        ...
        writeFileSync(
          join(dotOut, 'outcomes.json'),
          JSON.stringify({ caught: 7, timeout: 1, missed: 2, unviable: 5 })
        )
        return 0
      }
    }
  ...
  test('падає якщо llvm-cov exit ≠ 0 — explainer з install-командою', async () => {
    const dir = makeFixture()
    const runner = {
      async runLlvmCov() { return { exitCode: 1, stdout: '' } },
      async runCargoMutants() { return 0 }
    }
  ...
  test('падає якщо cargo-mutants не залишив outcomes.json', async () => {
    ...
    const runner = {
      async runLlvmCov() {
        return {
          exitCode: 0,
          stdout: JSON.stringify({ data: [{ totals: { lines: { covered: 0, count: 0 }, functions: { covered: 0, count: 0 } } }] })
        }
      },
      async runCargoMutants() { return 0 }  // outcomes.json не пишеться
    }
...
const defaultRunner = {
  async runLlvmCov({ manifestPath }) {
    const proc = Bun.spawn(
      ['cargo', 'llvm-cov', '--manifest-path', manifestPath, '--json', '--summary-only'],
  ...
  async runCargoMutants({ manifestPath, outDir }) {
    const proc = Bun.spawn(
      ['cargo', 'mutants', '--in-place', '-o', outDir, '--manifest-path', manifestPath],
...
function buildTotalsRow(rows) {
  const totalCoverage = rows.reduce(
    (acc, row) => addCoverage(acc, row.coverage),
...
## PR2 — `mlmail` cleanup (після релізу `@nitra/cursor@1.17.0`)
...
**Files:**
- Modify: `package.json` (mlmail root)
- Modify: `bun.lockb`
...
### Task 16: Оновити `package.json` (корінь mlmail)
...
### Task 17: Оновити `app/package.json` (mlmail workspace)
