...
const SCHEMA = JSON.parse(
  readFileSync(
    join(dirname(fileURLToPath(import.meta.url)), '../../schemas/rule-meta.json'),
    'utf8'
  )
)
...
describe('rule-meta.json lint fields', () => {
  ...
  test('accepts meta with all lint fields', () => {
    expect(validate({
      lint: 'quick', lintCmd: 'n-cursor lint-ga',
      lintScoped: false, lintAlways: false,
...
export function check(rulesDir) {
  ...
  for (const entry of entries) {
    ...
    if (!VALID_LINT_VALUES.has(meta.lint)) {
      violations.push(
        `rules/${id}/meta.json: \`lint\` має бути "quick" або "ci", отримано "${meta.lint}"`
      )
    }

    if (!meta.lintCmd) {
      violations.push(
        `rules/${id}/meta.json: поле \`lintCmd\` обовʼязкове коли є \`lint\``
      )
    }
  }
...
```json
{ "auto": { "glob": ["**/*.mjs", "**/*.cjs", "**/*.js", "**/*.jsx", "**/*.ts", "**/*.tsx"] }, "lint": "quick", "lintCmd": "n-cursor lint-js", "lintScoped": true, "lintCiCmd": "bunx jscpd . && bunx knip --no-config-hints" }
```
...
```json
{ "auto": { "glob": ["**/*.css", "**/*.vue"] }, "lint": "quick", "lintCmd": "npx stylelint '**/*.{css,scss,vue}' --fix" }
```
...
describe('runLintQuick', () => {
  test('runs quick steps only, passes changed files to scoped steps', () => {
    ...
    try {
      ...
      const code = runLintQuick({
        cwd: '/proj', spawnSyncFn, log: noop, logError: noop,
        rulesDir: root,
      })
      expect(code).toBe(0)
      // git diff called
  ...
  test('does NOT run lintCiCmd in quick mode', () => {
    const { root, cleanup } = makeRulesDir({
      jslint: { lint: 'quick', lintCmd: 'n-cursor lint-js', lintCiCmd: 'bunx jscpd .', lintScoped: true, lintAlways: true },
    })
    try {
      const { spawnSyncFn, calls } = makeSpawn([
        { status: 0, stdout: '' }, // git diff
