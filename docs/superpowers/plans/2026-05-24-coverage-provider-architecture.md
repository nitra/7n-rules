...
**Goal:** Перенести логіку `mail app/scripts/coverage.js` в `@nitra/cursor` як CLI-команду `n-cursor coverage` з провайдерами-плагінами, що активуються за `.n-cursor.json#rules`.
...
- [ ] **Step 3: Перевірити що команда визначається**
...
### Task 9: mail app cleanup (окрема PR — виконувати після публікації `@nitra/cursor@1.17.0`)
...
```bash
ls /Users/vitaliytv/www/vitaliytv/mail app/scripts/__tests__/
```
...
- [ ] **Step 2: Bumрпропустити `@nitra/cursor` до 1.17.0 у mail app**

```bash
cd /Users/vitaliytv/www/vitaliytv/mail app && bun add -D @nitra/cursor@1.17.0
```
...
```bash
rm /Users/vitaliytv/www/vitaliytv/mail app/scripts/coverage.js
rm /Users/vitaliytv/www/vitaliytv/mail app/scripts/with-lock.js
rm -rf /Users/vitaliytv/www/vitaliytv/mail app/scripts/__tests__/
```

- [ ] **Step 4: Оновити кореневий `package.json`**

Знайди у `/Users/vitaliytv/www/vitaliytv/mail app/package.json`:
...
Знайди у `/Users/vitaliytv/www/vitaliytv/mail app/app/package.json` і видали обидва рядки:
...
У `/Users/vitaliytv/www/vitaliytv/mail app/.n-cursor.json` додай `"test"` у масив `rules`.

- [ ] **Step 7: Верифікувати `bun run coverage`**

```bash
cd /Users/vitaliytv/www/vitaliytv/mail app && bun run coverage
```
...
- [ ] **Step 8: Запустити весь лінт mail app**

```bash
cd /Users/vitaliytv/www/vitaliytv/mail app && bun run lint
```
