...
- **Р11 — порт ≠ дзеркало реалізації.** Мета міграції — **ефективне
  ...
  5. **Виявлений дефект канону не копіюється.** Якщо під час порту видно, що
     ...
     антипатерн.
- **Р12 — інверсія мосту; napi більше не росте.** (Корекція черговості
  2026-08-03.) Досі порядок був: JS оркеструє → кличе Rust через napi. Наслідок
  — кожен порт робився двічі: спершу заводилась napi-поверхня, щоб концерн
  побачив JS-CLI, а потім фаза 8 зносила і napi, і JS-CLI. `findK8sRoots`/
  `findK8sYamlFiles` (PR #368) — приклад такої поверхні на викид.

  Правильний порядок зворотний: **`rules-cli` стає основним виконавцем `lint`,
  а непортовані JS-концерни виконуються як його дочірні процеси**. Rust
  оркеструє (`lint_plan`, `lint_render`, batch — уже в `rules-core`), node
  лишається дурним виконавцем залишку і скорочується з кожним портом.

  Практичні правила:
  1. **Нова native-функціональність у napi не заводиться.** Концерни й хелпери
     підключаються прямо в `rules-cli`/`rules-core`. Якщо здається, що потрібен
     napi-експорт — це сигнал, що робота йде в старому порядку.
  2. **Чинні 25 napi-експортів лишаються рівно як backend публічних JS-фасадів**
     (`@7n/rules/scripts/*` — 221 файл-споживач, Р6) для тих 96 концернів, що
     ще мають виконуваний `main.mjs`. Межа виводиться разом з останнім із них,
     а не окремим зусиллям.
  3. **Ціна мосту вимірюється, а не припускається.** Спавн node коштує ~40 мс
     старту (вимірювання зрізу 4 фази 8); для залишку концернів це один
     батч-процес, а не процес на концерн.
...
з явним переліком, що скорочується до нуля.

**Черговість (корекція 2026-08-03, Р12).** `lint` у Rust-CLI робиться **раніше**
за подальші порти концернів, а не після них: інакше кожен наступний концерн
платить за napi-поверхню, яку ця ж фаза й зносить. Порядок — спершу довести
`rules-cli lint` до основного шляху виконання (оркестрація вже в `rules-core`:
`lint_plan`, `lint_render`, `concerns::batch`), поставивши **зворотний міст**
до 96 концернів із виконуваним `main.mjs` — один дочірній node-процес на батч,
не на концерн. Далі кожен порт просто прибирає запис із того мосту.

**Ревізія рішення про `clap` (2026-08-03).** Зріз 3 відкинув `clap` на підставі
«кожна native-команда мусить byte-exact дзеркалити СВІЙ JS-парсер, а вони різні
за контрактом». Р11 (п. 2 і 4) цю підставу знімає: розбіжності JS-парсерів
(`changed-files` — fail-closed на невідомий аргумент, `rename-yaml-extensions` —
мовчазне ігнорування і `--root=` лише через `=`, `ci plan` — `indexOf`-семантика)
є **JS-епохи випадковістю**, а не вимогою; зовні їх ніхто не споживає, бо
єдиний споживач — JS-CLI, який виводиться. Отже `clap` повертається як
дефолт (Р11 п. 1: бібліотека виграє в ручного порту), а різнобій граматик
уніфікується — з change-файлом, бо для когось із користувачів це зміна
поведінки CLI.
