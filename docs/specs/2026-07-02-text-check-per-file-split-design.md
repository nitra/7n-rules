...
| ------------------------- | -------------------------------------- | ----------------------- | ----------- | ---------------- |
| `style/lint` | `style/lint/` (без змін цією спекою) | `stylelint --fix` | [fix-lint.mjs:45-65](npm/rules/style/lint/fix-lint.mjs) | `apply()` бере файли з `ctx.files` (delta) чи `git ls-files` (full) — НЕ з `violations` |
...
| `rust/check` | `rust/check/` (лишається bundled — див. "Не в цій спеці") | лише **`cargo fmt --all`**-крок | [fix-check.mjs:39-58](npm/rules/rust/check/fix-check.mjs:39) | `apply()` re-lists `**/*.rs` через git, не з violations; `clippy`/`cargo deny` **НЕ мерджаться** — clippy `--fix` свідомо не автоматизований (коментар у коді: "потенційно небезпечний"), deny.toml-генерація — окремий T0-патерн поза цим списком. Тобто merge стосується лише fmt-підмножини вже bundled `rust/check`, не всього concern-а |

> [!WARNING]
> **Виключено після реалізації (корекція аудиту):** `js/eslint` і `text/oxfmt` **не** мерджаться, хоча первинний аудит їх туди зараховував. Обидва `apply()` деривують список цільових файлів **з самих `violations`** (`violations.map(v => v.file)` / `violations.filter(...).map(v => v.file)`), а не самостійно перелічують файли (`git ls-files`/`ctx.files`, як усі інші 6). Якщо викликати такий `apply()` з порожнім `violations: []` (а саме так це відбувається без початкового detect) — список файлів завжди порожній, і фікс **ніколи не запускається**. Первинне формулювання критерію ("apply() бере лише `v.file`, не координати") було неточним: воно описувало, ЩО саме береться з violations, а не ЧИ можна викликати `apply()` без них узагалі. Виправлений критерій (в) додано нижче.
...
**Уточнений критерій (в):** (а) `apply()` re-invoke'ить CLI, що сам re-scan'ить вміст; (б) жодне поле з `violation.data` не читається; **(в) сам список цільових файлів `apply()` бере НЕ з `violations`, а самостійно** (`git ls-files`/`ctx.files`/glob) — інакше виклик з порожнім `violations: []` (без початкового detect) ніколи не знайде, що фіксити.
...
Фінальний перелік (реалізовано, `standalone: true` у 2026-07-02): **6 concerns** — `text/markdownlint`, `text/run-shellcheck`, `text/run-dotenv-linter`, `python/ruff`, `style/lint`, `rust/check` (fmt-підмножина). `js/eslint` і `text/oxfmt` виключено (див. WARNING вище). Реалізація — `npm/scripts/lib/lint-surface/run-fix.mjs` (`T0Pattern.standalone`, `isStandaloneConcern`, `runFixPipeline` пропускає початковий detect для повністю standalone-концернів); тест підтверджує економію (1 виклик `lint()` замість 2 на "брудному" шляху) в `run-fix.test.mjs`.
