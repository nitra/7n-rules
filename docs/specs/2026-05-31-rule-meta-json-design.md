...
- `npm/scripts/tests/auto-rules.test.mjs` — **зберегти всі ~50 кейсів** (вони перевіряють _поведінку_ `detectAutoRules`, не внутрішню реалізацію). Оновити лише фікстури, якщо тест створював `auto.md`. Додати кейс для **tauri** (тепер автодетектиться). Усі наявні assertions щодо порядку/детекту мають лишитись зеленими — це головний регресійний контракт.
