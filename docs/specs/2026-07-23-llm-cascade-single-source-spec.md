...
| З.1 | Pi-пресет (за R1; тіри переглянуто 2026-07-27) | Спавн: `npx -y pi-acp` (npm-пакет **`pi-acp`**, без scope; він сам спавнить `pi --mode rpc`). Тір-механізм — **не** env/args на спавні, а протокольний виклик `session/set_config_option` (`configId: "model"`, `value: "<provider>/<modelId>"`) між `session/new` і `session/prompt` — session-шар крейта має підтримувати опційний post-session-creation config-крок. Тіри (оновлено 2026-07-27, llm-lib v0.2.3: min/avg — локальні/безкоштовні, max — підписка): MIN = `omlx/gemma-4-e4b-it-OptiQ-4bit`, AVG = `litellm/gemma-4-26b-awq` (LiteLLM-проксі `https://llm.7n.ai/v1/` → `llm-d`, див. PR #249), MAX = `openai-codex/gpt-5.6-sol`. Передумови на машині: провайдери `omlx`/`litellm` описані в pi (`~/.pi/agent/models.json`; litellm потребує ключа), для MAX — codex-логін; без відповідного провайдера `session/set_config_option` (а отже спавн сесії з цим тіром) провалиться |
