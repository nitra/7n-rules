...
- **Ф1 — pi-runner (один PR):** `pi-agent-skill.mjs`; `RUNNERS` += `pi` (дефолтний executor); `runLlmCli` → роутинг pi/claude/cursor із deprecation-warning на claude/cursor; поле `tier` у схемі `main.json` + дефолт `max`; читання `tier` у runner; telemetry `kind:"skill"`; тести (unit + scripted-provider). claude/cursor-гілки лишаються робочими.
...
1. **Точна назва `bash`-tool у pi-API** — звірити `createBashTool` / `createBashToolDefinition` vs рядковий `'bash'` у `tools:[]` при реалізації (експорти підтверджені: `createBashTool`, `createBashToolDefinition`, `BashExecutionComponent`, `createLocalBashOperations`).
2. **`worktree: true`-скіли.** `taze`/`lint`/`adr-normalize` мають `worktree: true` у `main.json` (запуск лише в окремому git-worktree). Як `runPiAgentSkill` узгоджується з worktree-preflight скіла (хто створює worktree — CLI чи сам скіл-агент): уточнити при реалізації Ф1, можливо `cwd` сесії = worktree-корінь.
3. **Turn-ceiling для скілів** — `~80` орієнтовно; відкалібрувати за смоуком (скіли об'ємніші за фікс одного правила).
4. **pi-native skill-loading** (`loadSkills`/`formatSkillsForPrompt`) — можливий майбутній рефактор, якщо захочемо скіли як first-class pi-концепт (зараз свідомо ні, §3).
5. **Стартовий маппінг тир** per-skill — у Ф2, не в спеці.
