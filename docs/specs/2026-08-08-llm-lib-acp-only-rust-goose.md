...
залежність. Шляхи виду `llm-lib/crates/llm-lib/src/...` нижче по тексту
читати як шляхи в тому окремому репозиторії.

**Стан на 2026-08-19:** виділений репозиторій став workspace-ом
(`llm-lib` 0.3.0, **`harness` 0.1.0** — вся оркестрація pipeline/ladder/
snapshot/collateral/test_gate переїхала туди з `llm_lib::fix::*`, `trace`,
`plugin-host`, `gix-util`); `Tier` плоский, T0 — декларативний `EditPlan`.
Теку `llm-lib/` у цьому репозиторії видалено ЦІЛКОМ: napi-крейт помер, як і
планувала структура вище, а JS-пакет `@7n/llm-lib` **заморожено на 3.4.0 з
реєстру** (споживачі §3.3 живуть на ньому до своїх Rust-зрізів; джерело
відновлюване з git-історії). Крейти `rules-*` досі пінять `n7n-llm-lib`
0.2.4 — breaking-міграція на 0.3.0 + `n7n-harness` описана в
[docs/plans/2026-08-19-llm-lib-0.3-harness-migration.md](../plans/2026-08-19-llm-lib-0.3-harness-migration.md)
(реєстр, §5.0).
