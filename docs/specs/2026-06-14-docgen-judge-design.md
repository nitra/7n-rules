...
- Сильно вище порогу «>15% → будувати». `generic`=0% підтверджує: будувати **тільки `inaccurate`-гейт**, R4/E2 для generic достатньо.
- Помилки **системні й перевірені** (3 спот-чеки проти коду — суддя правий): майже всі — галюцинована секція «Гарантії поведінки»:
  - вигадана error-поведінка: «fail-safe, повертає `false`/`null`/`Err`», тоді як код кидає `Error` / повертає масив (dev-link, sources, python/sql/vue-chunker, client, dependency-extractor);
  - хибне «без мережевих звернень», тоді як код робить `graphQLClient.request`/GitHub API (comment, cli-sources, sources);
  - **інверсія логіки безпеки** (allowlist.js): Огляд каже «без allowlist → доступ заборонено», код — `return true` (дозволяє). Док навіть сам собі суперечить (Огляд ≠ Поведінка).

**Висновок — два діяння (за пріоритетом доктрини `programmatic-checks-for-llm.md`):**

1. **Спершу — детерміновані правила на системні патерни (0 токенів, найбільший ROI).** Вимір показав, що галюцинації НЕ випадкові — це шаблонний «fail-safe/no-network» бойлерплейт у «Гарантіях». Узагальнити наявне `cache-hallucination` правило R5+ у `scoreDoc`:
   - `false-error-return`: док стверджує повернення `false`/`null`/`Err` на помилку, а в `facts` нема такого return / є `throw` → штраф;
   - `false-no-network`: док стверджує відсутність мережі, а `facts.markers`/імпорти містять `graphQLClient`/`fetch`/GitHub-клієнт → штраф.
   - Альтернатива/доповнення: **обмежити саму генерацію** секції «Гарантії поведінки» (модель її вигадує) — emit лише anchor-verifiable гарантії. Це може зняти більшість FP на корені, без жодного гейта.
2. **Потім — `inaccurate`-judge-гейт (цей spec)** на незводимий хвіст, що лишиться після (1). Scope строго `inaccurate`; модель — хмарна (як у вимірі); гейтити смугою + кеш.

Тобто judge підтвердив свою цінність насамперед **як discovery-tool**: він виявив системні патерни, які тепер дешевше прибити детерміновано — рівно за твоєю доктриною.
