...
`crates/rules-docs` (14 410 рядків Rust — окрема поверхня, яка чекає на
ПРОВОДКУ, не на порт: вона підключена в `crates/rules-cli`
(`Cargo.toml`, `src/docs_cmd.rs`) і досяжна командою `n-rules docs`, але
підкоманди `domains`/`index`/`slice`/`validate` йдуть у Rust лише за
прапорцем `--native-docs`, тобто НЕ дефолтом. Називати її «мертвою» —
хибно: код виконуваний, змінений лише дефолт маршрутизації, і з цієї
неточності легко вивести хибний висновок «можна знести 14 410 рядків»).
`n-rules docs`
