...
Цей документ пропонує зміну парадигми від LLM-інтерпретації правил до їхньої програмної верифікації через CLI-скрипти. Основна гарантія — значне зниження використання токенів та часу, оскільки замість десятків викликів інструментів для ручного аналізу файлів, агент отримує структурований результат від одного детермінованого виклику. Це дозволяє забезпечити повну дотриманість конвенцій проєкту без надмірного навантаження на мовну модель.
...
Кожен tool call — це overhead: токени на запит, відповідь, reasoning. І це лише **одне** правило. При кількох правилах агент витрачає 15-20 tool calls і тисячі токенів на те, що `node` зробить за 100ms.
...
`npx @nitra/cursor check` без аргументів сканує `.cursor/rules/*.mdc` і запускає лише ті перевірки, для яких у пакеті є відповідний check/policy (наприклад `n-bun.mdc` → `check bun`). Явний список у командному рядку лишається можливим: `npx @nitra/cursor check bun ga`.
...
Виміряно на проєкті з 9 правилами (bun, ga, js-lint, text, style-lint, npm-module, js-run, nginx, vue):
...
### Верифікація одного правила (text / oxfmt)
