//! **JS-варіант** гейта кроку 6 (доккомент
...
//! ІНШОМУ комбінованому world-і (`js-coverage-provider-guest.wit`,
//! guest-фікстура тут НЕ споживає реальний `crates/plugin-lang-js`, лише
//! доводить сам контракт «declared_worlds → доступ до collect-coverage» на
//! мінімальній фікстурі, ЯК і rust/python/php-версії цього гейта). ЧЕТВЕРТИЙ
//! і останній провайдер спеки §12 (§2.123 реєстру відкритих питань винесла
//! цей гість окремою хвилею — 46 файлів канону, п'ятиразово більший обсяг
//! за решту трьох разом).
//!
...
//!
//! # ДРУГА структурна різниця, специфічна для JS — негативний тест НЕ через `load()`
//!
//! Рust/python/php-гейти доводять негативну половину через
//! `PluginHost::load` (порожній `declared_worlds`) — їхній world несе лише
//! `plugin` + `coverage-provider`, і `coverage-provider` не має жодного
//! import-у (доккомент `link_coverage_provider`), тож інстанціація без
//! ЖОДНОГО заявленого world-а все одно проходить. `js-coverage-provider-guest`
//! — ТРИЙНИЙ world (`plugin` + `n-rules:caps/file-reader@1.0.0` +
//! `n-rules:surfaces/coverage-provider@1.0.0`, той самий, що РЕАЛЬНИЙ
//! `crates/plugin-lang-js`): file-reader — світ повноважень, і компонент,
//! зібраний проти нього, СТРУКТУРНО імпортує `list-files`/`read-file-bytes`
//! незалежно від того, чи гість реально їх викликає (доккомент
//! `js-coverage-provider-guest.wit`: Component Model не має «опціональних»
//! імпортів). Тобто `PluginHost::load` (без жодного world-а) провалив би
//! ІНСТАНЦІАЦІЮ тут — не тому, що coverage-provider не заявлений, а тому,
//! що лінкеру бракує file-reader. Негативний тест цього файлу тому
//! використовує `load_for_worlds` з РІВНО одним заявленим world-ом —
//! `n-rules:caps/file-reader@1.0.0` (потрібен структурно для інстанціації),
//! БЕЗ `coverage-provider` — точний відповідник стану `plugin-lang-js` ДО
//! цього кроку (крок 5: `worlds = ["n-rules:caps/file-reader@1.0.0"]`).
//! Це не послаблення негативної половини — навпаки, точніший доказ: він
//! показує, що декларація ОДНОГО world-а не «протікає» дозвіл на ІНШИЙ.
//!
...
//! збирається проти `crates/rules-contract/wit/js-coverage-provider-guest.wit`
//! — ПОСТІЙНОГО файлу цього кроку, того самого, що споживає РЕАЛЬНИЙ
//! `crates/plugin-lang-js`. Копіювання дерева в tempdir тому не потрібне —
...
const CRATE_NAME: &str = "coverage-provider-gate-fixture-js";
const PLUGIN_ID: &str = "coverage-provider-gate-js/probe";
const CONCERN_ID: &str = "coverage-provider-gate-js/probe";
...
const REPORT_AREA_MARKER: &str = "GATE-MARKER-Js";
...
/// Guest-крейт цього гейта: зібраний проти ТРИЙНОГО world-а (доккомент
/// модуля), на `collect-coverage` повертає канонічний звіт із
/// [`REPORT_AREA_MARKER`] у назві виміру (echo `request.cwd`, щоб довести
/// і round-trip входу) — АБО типізовану помилку за [`TRIGGER_FAILED`] у
/// `mutation-refresh-files`. `list-files`/`read-file-bytes`
/// (file-reader-імпорти, частина типу компонента незалежно від того, чи
/// гість їх викликає) — НЕ використовуються тілом жодного методу: цей
/// гейт доводить лише механізм `coverage-provider`, file-reader тут
/// присутній структурно (доккомент модуля, розділ «ДРУГА структурна
/// різниця»), не як предмет доказу.
...
/// модуля, розділ «Комбінований world — ПОСТІЙНИЙ»): `js-coverage-provider-guest`
...
/// Скаффолдить і збирає гостя РАЗ на весь тестовий бінар (усі тести цього
/// файлу ділять той самий `.wasm`) — той самий прийом, що
...
fn fixture_wasm() -> &'static Path {
    ...
    WASM.get_or_init(|| {
        ...
        assert!(
            ...
            "гість-фікстура coverage-provider-gate-js не зібралась:\n--- stdout ---\n{}\n--- stderr ---\n{}",
...
/// `declared_worlds` (ОБИДВА world-и — file-reader структурно потрібен для
/// інстанціації, доккомент модуля), дає РЕАЛЬНИЙ звіт покриття — `area`
/// несе [`REPORT_AREA_MARKER`] і echo `cwd` запиту, підтверджуючи, що дані
...
fn declares_world_collects_coverage_through_guest_export() {
    ...
    let mut plugin = host()
        ...
        .expect("гість, що оголосив обидва world-и, має інстанціюватись");
...
/// завантажений із заявленим ЛИШЕ `n-rules:caps/file-reader@1.0.0` — БЕЗ
/// `n-rules:surfaces/coverage-provider@1.0.0` у `declared_worlds`. File-reader
/// заявлений, бо структурно потрібен для інстанціації (доккомент модуля,
/// розділ «ДРУГА структурна різниця» — на відміну від rust/python/php тут
/// `PluginHost::load` без жодного world-а провалив би саму інстанціацію,
/// не лише виклик collect-coverage). Інстанціація проходить (export
/// нікуди не дівається), але [`LoadedPlugin::collect_coverage`] відмовляє
/// ГУЧНО, типізовано (`PluginHostError::SurfaceNotDeclared`) — не мовчки
/// повертає порожній звіт (правило проєкту, доккомент [`crate`]).
#[test]
fn undeclared_world_refuses_to_call_export_loudly() {
    let wasm = fixture_wasm();

    let mut plugin = host()
        .load_for_worlds(wasm, PLUGIN_WORLD_VERSION, &[FILE_READER_WORLD.to_string()])
        .expect(
            "гість інстанціюється, коли задекларовано лише file-reader \
             (coverage-provider не має жодного імпорту й на інстанціацію не впливає)",
        );
