//! **Python-варіант** гейта кроку 6 (доккомент
...
//! ІНШОМУ комбінованому world-і (python-coverage-provider-guest.wit,
//! guest-фікстура тут НЕ споживає реальний `crates/plugin-lang-python`,
...
//! збирається проти `crates/rules-contract/wit/python-coverage-provider-guest.wit`
//! — ПОСТІЙНОГО файлу цього кроку, того самого, що споживає РЕАЛЬНИЙ
//! `crates/plugin-lang-python`. Копіювання дерева в tempdir тому не потрібне
...
const CRATE_NAME: &str = "coverage-provider-gate-fixture-python";
const PLUGIN_ID: &str = "coverage-provider-gate-python/probe";
const CONCERN_ID: &str = "coverage-provider-gate-python/probe";
...
const REPORT_AREA_MARKER: &str = "GATE-MARKER-Python";
...
/// модуля, розділ «Комбінований world — ПОСТІЙНИЙ»): `python-coverage-provider-guest`
