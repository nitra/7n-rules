...
//! збирається проти `crates/rules-contract/wit/rust-coverage-provider-guest.wit`
//! — ПОСТІЙНОГО файлу цього кроку, того самого, що споживає РЕАЛЬНИЙ
//! `crates/plugin-lang-rust`. Копіювання дерева в tempdir тому не потрібне
...
const CRATE_NAME: &str = "coverage-provider-gate-fixture";
const PLUGIN_ID: &str = "coverage-provider-gate/probe";
const CONCERN_ID: &str = "coverage-provider-gate/probe";
...
const REPORT_AREA_MARKER: &str = "GATE-MARKER-Rust";
...
/// модуля, розділ «Комбінований world — ПОСТІЙНИЙ»): `rust-coverage-provider-guest`
