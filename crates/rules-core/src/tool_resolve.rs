...
//! детермінованим ядром лінту, а не встановлювачем тулів. Тому native-шар
//! резолвить **уже встановлений** тул.
//!
//! # Що робить концерн, коли тул не резолвиться
//!
//! **Fail-closed**: концерн повертає помилку з install-підказкою, і прогін
//! падає (exit 2). Це рівно чинний режим `N_CURSOR_NO_AUTO_INSTALL`
//! (`ensure-tool.mjs:533-538`), а не новий стан: `ensureTool` уже вміє
//! відмовитись від авто-встановлення й віддати ту саму per-OS підказку.
//! Мовчазний пропуск (0 violations) був би fail-open — на ефемерному
//! CI-раннері з порожнім кешем тул майже завжди відсутній, і перевірка
//! просто зникла б.
//!
//! Добування тулів переїжджає в окрему команду `tools ensure` (окремий
//! зріз міграції) — саме тому lint-рантайм тут навмисно нічого не
//! встановлює, а лише резолвить і чесно каже, чого бракує.
//!
...
//! клас виклику, що й ручний скан. Р11 п.1 і п.2 спеки: зрілий крейт
//! виграє в ручного порту, а JS-обхід чужого обмеження не переїжджає.
//!
...
/// такої семантики тут бути не повинно.
...
/// `None` = тула немає; викликач зобовʼязаний упасти fail-closed із
/// підказкою ([`install_hint`]), а не пропустити перевірку.
...
/// Per-OS install-підказка для fail-closed повідомлення — порт `buildHint`
/// (`ensure-tool.mjs:495-509`). `brew`/`scoop`/`github` — ті самі поля
/// запису реєстру `TOOLS`, які викликач передає явно (реєстр тулів у
/// `rules-core` не дублюється: тут лише один концерн-споживач, і власна
/// копія всієї таблиці розійшлась би з JS-каноном непомітно).
pub fn install_hint(tool_id: &str, brew: &str, scoop: Option<&str>, github: &str) -> String {
    let mut lines = vec![
        format!("{tool_id} не знайдено ні в PATH, ні в керованому кеші бінарників."),
        "   Встанови:".to_string(),
    ];
    if cfg!(target_os = "macos") {
        lines.push(format!("     macOS: brew install {brew}"));
    } else if cfg!(windows) {
        if let Some(scoop) = scoop {
            lines.push(format!("     Windows: scoop install {scoop}"));
        }
        lines.push(format!("     або: https://github.com/{github}/releases"));
    } else {
        lines.push(format!("     Linux: https://github.com/{github}/releases"));
    }
    lines.join("\n")
}
...
mod tests {
    ...
    /// Ні в PATH, ні в кеші → `None` (сигнал «падай fail-closed із підказкою»).
    #[test]
    ...
    /// Підказка називає тул і дає щонайменше один спосіб встановлення для
    /// поточної ОС — це той мінімум, за яким fail-closed повідомлення
    /// лишається дієвим, а не просто «немає тула».
    #[test]
    fn install_hint_names_tool_and_offers_a_route() {
        ...
        let has_route = hint.contains("brew install")
            || hint.contains("scoop install")
            || hint.contains("github.com/");
        assert!(has_route, "{hint}");
    }
}
