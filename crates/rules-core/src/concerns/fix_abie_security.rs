...
//! на рівні типу. УВАГА (важливо для оркестратора): виконавець планів
//! (`crates/rules-fix/src/t0.rs::apply_plan`) наразі вміє лише
//! `std::fs::remove_file` — на директорії ця операція мовчки провалюється
//! (`apply_plan` рахує таку правку як «не застосовано», не панікує й не
//! блокує решту плану — той самий контракт, що тест
//! `one_failing_edit_does_not_block_the_rest_of_the_plan` у `t0.rs`).
//! Видалення директорій (`std::fs::remove_dir_all` для `Delete`-шляхів, що
//! насправді вказують на директорію) — окрема правка `apply_plan`, не цього
//! файлу; до неї `Delete`-запис на `.firebase/` у плані є коректним
//! ДЕКЛАРАТИВНО, але не виконається цим виконавцем.
//!
...
mod tests {
    ...
    /// (секція доккоменту модуля про обмеження `apply_plan`).
    #[test]
