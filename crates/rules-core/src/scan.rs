...
//! `crates/rules-napi/src/lib.rs::walk_dir`); і обхід ПІДДЕРЕВА, що вже
//! пройшло фільтрацію на рівні вибору коренів (kubescape, доккомент
//! `k8s_manifests_kubescape.rs`).
//!
//! Реєстр §2.27 лишив ЩЕ два «1:1-порти» відкритим питанням —
//! `sample_secret`/`hasura_migrations`, чиї JS-оригінали самі не
//! консультувались із consumer-ignore. §2.30 його закрило: обидва тепер
//! ідуть через обгортки (`walk_repo`/`walk_under_repo` відповідно, доккомент
//! кожного модуля) — прямих викликів [`walk_dir_raw`] у них більше нема.
//!
//! # Двигун
//!
