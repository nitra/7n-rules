...
//! golden-тести). ДЕВʼЯТЬ концернів у контрибуції, порт чинних JS-оригіналів
//! — справжній 1:1, той самий `reason`/`message` біт-у-біт (parity-дисципліна
//! СКІЛа не допускає shadowing regex-наближенням AST-оригіналу в контрибуції
//! — рішення оркестратора після звіту батчу 2, доккомент секції «Регекс-
//! наближення» нижче):
//!
...
//!
//! # Девʼять концернів в одному Guest — мотив із `test-plugin-guest`
//!
...
/// Guest-реалізація world `plugin` — девʼять контрибуцій ([`CONCERN_TFM`],
...
mod tests {
    ...
    #[test]
    fn build_manifest_declares_all_nine_concerns_with_expected_scopes() {
