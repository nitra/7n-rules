...
//!
//! # §2.90 — цей крейт ЄДИНА реалізація фіксу сімнадцяти концернів
//!
//! Борг «спершу парність» для `plugins/ci-github` закрито: сімнадцять
//! `fix-<concern>.mjs` (усі, крім `ga/service_deploy_workflow` і
//! `ci_artifact/consume` — розділ «Портовано ЛИШЕ detect» нижче) ЗНЯТО.
//! Наслідок, не деталь: `loadT0Patterns` (`npm/scripts/lib/lint-surface/
//! run-fix.mjs`) резолвила фіксери native → wasm (`guestFix`) →
//! `fix-<concern>.mjs`, і третій шар був глушником випадку «гість не
//! резолвиться» (плагін не зібрано, розбіжність піна, хост без wasm).
//! Глушника більше НЕМАЄ — якщо цей гість не зарезолвиться, концерн
//! деградує з «автофікс» у «повідомили й віддали в LLM-ладдер». Тому
//! КОЖНА гілка фіксера, що віддає порожній план, мусить бути СВІДОМИМ
//! no-op, а не мовчазним «підхопить JS»: читання «порожній план →
//! підхопить канон» відтоді НЕПРАВДИВЕ скрізь, крім
//! `ga/service_deploy_workflow`. Склад резолву стереже табличний гейт
//! `§2.90 …фікс кожного портованого концерну живе рівно в одному місці`
//! (`wasm-plugin-parity-ci-github.test.mjs`): рівно один патерн на
//! концерн, і той `guestFix`.
//!
...
// `fix-toolchain_cache.mjs` (файл ЗНЯТО §2.90 — цей крейт відтоді ЄДИНА
// реалізація фіксу концерну; назви JS-символів нижче лишаються як опис
// джерела порту, читати їх треба в історії git):
