...
//! звіт українською.
//!
//! # Прапорець лишився, дефолт — ні (крок 3 плану full-rust-migration)
//!
//! До кроку 3 `--native-fix` був опційним вмикачем: без нього `lint` без
//! `--no-fix` ішов у JS-CLI, де живе чинний fix-пайплайн, навіть коли
//! позиційні аргументи вже були ключами `rule/concern`, які цей модуль умів
//! обробити. Тепер [`crate::lint_cmd::run`] розпізнає ту саму форму виклику
//! сам (усі `rules` — ключі `rule/concern`, без `--no-fix`) і кличе цей
//! модуль без прапорця; `--native-fix` лишається явним форс-вмикачем (і
//! єдиним шляхом, коли форма виклику не збігається з евристикою), а новий
//! `--no-native-fix` — аварійним люком назад у JS.
//!
//! **Що НЕ покриває цей перемикач.** Голий `n-rules lint` (без позиційних
//! ключів) — це план УСЬОГО прогону, а не перелік concern-ів: цей модуль
//! приймає рівно перелік `rule/concern`-ключів ([`concern_keys`]), не план
//! ([`crate::lint_plan`]), і `--no-fix` цей модуль взагалі не читає (не
//! його турбота — прапорець просто вимикає виклик модуля). Дефолтний
//! fix-пайплайн ГОЛОГО `lint` — те, що ще виконують `ladder.mjs`/
//! `snapshot.mjs`/`collateral-veto.mjs`/`test-gate.mjs` (§2.104 реєстру
//! відкритих питань) — лишається JS-поверхнею: перемкнути й це означало б
//! спершу побудувати native-оркестрацію «план → перелік порушених
//! concern-ів → `rules_fix::fix_concerns`», якої сьогодні немає.
//!
//! # Межі цього зрізу
//!
//! `concern_keys` приймає один АБО КІЛЬКА concern-ключів (`rule/concern`) —
//! не план усього прогону: `fix_concerns` (`rules_fix`) уже вміє список,
//! спільний кеп найдорожчого тиру рахується на весь виклик ([`run_blocking`]).
