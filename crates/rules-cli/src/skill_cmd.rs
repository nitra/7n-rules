...
/// Скіли з власним JS-оркестратором — конвеєр детермінованих кроків із
/// точковими LLM-викликами, не один агентний хід. **Порожньо.**
///
/// `taze` пішов звідси §2.125-патерном (§2.125,
...
/// блокер §3.3 того ж розбору: `js/`-підкаталоги скіла не копіюються в
/// дерево консюмера (`n-rules-cli.mjs:957-961`), а після зняття npm-каналу
/// (зріз 6) `js_fallback` не має куди їх делегувати — `skill … taze`
/// перестав би працювати мовчки-ламано. Розбір скіла: worktree-preflight
/// несе загальний `n-rules:worktree:start`-блок SKILL.md (той самий, що й
/// у будь-якого `worktree: true` скіла), кроки 1/3/7 (backup/diff/cleanup)
/// — нові команди `n-rules taze diff|backup|cleanup`
/// (`npm/bin/n-rules-cli.mjs`, дельта до вже наявного `taze diff`), крок 2
/// (bump) — літеральні шелл-команди у фрагменті плагіна, кроки 4-6
/// (breaking-changes-перевірка на один major-пакет) агент виконує сам,
/// послідовно, БЕЗ вкладеного ACP-виклику — той самий агент, що вже читає
/// `SKILL.md`, є раннером.
///
/// `git-reconcile` (3 484-рядковий `skills/git-reconcile/js/orchestrate.mjs`)
/// пішов тим самим шляхом, окремою хвилею (реєстр §2.135, рішення власника
/// §7 `docs/plans/2026-08-31-full-rust-migration-plan.md`, 2026-09-01).
/// Різниця з `taze`: конвеєр складніший — resumability після переривання
/// процесу під час незворотних Git-мутацій (branch/worktree/stash archive→
/// cleanup, вимога ADR #334) лишається кодом, не текстом, зі своїм
/// персистентним станом (`git_reconcile_cmd` — `archive`/`cleanup`/`gc`/
/// `restore`/`status`, доккомент модуля пояснює МЕЖУ текст/код детально).
/// Решта колишнього конвеєра — inventory, semantic triage, conflict
/// resolution, gates, PR-опис і верифікація checks — стала покроковими
/// інструкціями `SKILL.md`: той самий агент, що раніше викликався ІЗ JS,
/// тепер сам є раннером, як і в `taze`.
