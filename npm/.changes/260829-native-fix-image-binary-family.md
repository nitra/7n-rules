...
Ще заблокованим лишається другий споживач планів — місток T0 петлі `fix` (`crates/rules-fix`): `llm_lib::edit_plan::EditPlan` текстовий наскрізь, тож `write-bytes` там і далі дає гучну типізовану відмову. Байтовий `FileEditPlan` — робота в окремому репозиторії `llm-lib`, споживана тут лише після публікації крейта.
