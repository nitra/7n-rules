...
export async function runCli(argv) {
  ...
  try {
    if (isLintHelp) {
      ...
      // `skill <runner> <id>` більше НЕ має власного JS-оркестрованого
      // виключення (§2.125-патерн застосований до всіх скілів, що мали
      // власний конвеєр — `taze`, потім `git-reconcile`, реєстр §2.135):
      // немає окремого Git/worktree preflight, за яким self-upgrade мав би
      // почекати.
      ...
      const skipDevDepsEnsure =
        command === 'docs' || (command === 'lint' && isLintFullFixArgs(args))
