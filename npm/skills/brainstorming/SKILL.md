...
### 5. Дія — доведи сесію до явного рішення

- Топ-3-5 ідей отримують: короткий наступний крок і як зрозуміти що ідея "зайшла" (success signal).
- **Не закінчуй сесію на списку кандидатів.** Прямо запитай користувача, який варіант (або яку комбінацію) він обирає — мета кроку 5 не "показати опції", а **зафіксувати рішення**, яке піде у крок 6. Якщо користувач вагається між 2-3 варіантами — постав це як окреме питання з варіантами вибору, а не залишай відкритим.
- Після того як рішення явно підтверджено, запитай, чи він хоче:
...
Chosen option: "<варіант, який користувач явно підтвердив на кроці 5>", because <обґрунтування з кроку 4-5>.
...
Зберігай у `docs/superpowers/specs/YYYY-MM-DD-<slug-теми>.md`, **українською** (код/шляхи/API-назви — англійською). Формат — як у решті `docs/superpowers/specs/`: без MADR-секцій, натомість структура "проблема → ухвалені рішення → деталі реалізації", готова передаватись у розробку без додаткового уточнення.
...
| # | Питання | Рішення |
|---|---|---|
| А | <питання, яке розв'язав крок 4-5> | <обраний варіант і коротке обґрунтування> |
| Б | ... | ... |
...
- Якщо тека `docs/superpowers/specs/` відсутня — створи. Після запису — запропонуй git commit (`docs(spec): brainstorm session <slug>`), але не роби commit без підтвердження користувача.
