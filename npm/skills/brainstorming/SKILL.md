...
### 6. Збереження сесії — завжди у форматі ADR (MADR v4)

Результат сесії завжди оформлюється як ADR-документ у канонічному форматі проєкту (MADR v4) і зберігається в `docs/adr/<YYYYMMDD-HHMMSS>-<slug-теми>.md` (timestamp — момент запису, slug — латинізована коротка назва теми). Це той самий формат, що й у решті `docs/adr/` — тож файл одразу читається як частина архітектурної історії проєкту, без окремого проміжного "brainstorm-документа".
...
Chosen option: "<топ-ідея або топ-3-5, якщо користувач не звузив до однієї>", because <обґрунтування з кроку 4-5>.
...
- **Status** — `Proposed`, якщо користувач ще не підтвердив вибір явно; `Accepted`, якщо на кроці 5 користувач прямо погодився з конкретним варіантом.
...
- Якщо сесія закінчилась без єдиного переможця (кілька рівноцінних топ-ідей) — `Decision Outcome` формулюй як рекомендацію розглянути топ-N, а не вигадуй штучний "переможець".
