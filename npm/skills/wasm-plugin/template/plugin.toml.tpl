...
# УВАГА: усі поля рівня маніфесту (`ci_artifacts`, `tools`, і будь-які нові)
# мусять стояти ВИЩЕ за перший `[[concerns]]`. У TOML пара ключ-значення
# належить останній відкритій таблиці, тож нижче цього рядка вони стануть
# полями концерну, а не маніфесту — мовчки, без помилки парсера.
