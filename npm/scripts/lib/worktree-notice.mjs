...
function buildNoticeBody(suffix) {
  return `> [!IMPORTANT]
> **Worktree-only skill.** Виконується **виключно** в окремому git-worktree (\`.worktrees/<current-branch>-${suffix}/\`, де slash у назві замінено на \`-\`) і **не** паралелиться — один інстанс за раз.
...
\`\`\`bash
mt worktree create "feature-x-${suffix}" --description "n-${suffix}: worktree-only skill"
cd ".worktrees/feature-x-${suffix}"
\`\`\`

Тобто name для \`mt\` і шлях для \`cd\` беруть sanitized форму: slash → \`-\`; CLI створює власну git-гілку \`mt/feature-x-${suffix}\`.
