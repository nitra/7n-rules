...
B1 MT-tail (Фаза B, спека 2026-07-11): коли лишився невиправлений хвіст (worst=1), `renderRemaining` повертає зібрані порушення, і за env-гейтом `N_LINT_MT_TAIL=1` вони матеріалізуються у вузли MT-графа через `materializeTail` (mt-tail.mjs). Fail-open: MT недоступний або будь-яка помилка → лог, lint не падає; дефолт вимкнено (prod-enable — після MT executor-моста + addon-підпакетів).
