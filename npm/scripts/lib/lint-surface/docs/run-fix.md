...
Test-gate veto (addendum 2026-07-24): collateral-veto ловить лише правки ПОЗА target-set — правки ВСЕРЕДИНІ вже-таргетованого файлу (напр. видалений задокументований workaround поруч із фіксом порушення) не зачіпають жодного детектора й проходять і collateral-veto, і canonical re-detect. Коли `after.length === 0 && !error && collateral.length === 0`, `runRung` бере наявні файли з target-set, реально змінені rung-ом (`snapshot.modifiedExisting()` ∩ `resolveTargetSet`), і через `findBrokenSiblingTests` (`test-gate.mjs`) шукає й виконує їхні сестринські тест-файли (`<dir>/tests/<stem>.test.{mjs,js,ts}`, n-test.mdc). Провал такого тесту → той самий шлях rollback/feedback/телеметрія, що й collateral-veto, але `kind:"test-gate-veto"`. Fail-open за дизайном: відсутній test-runner, таймаут чи відсутність сестринського тесту — не блокує rung. `deps.testRunner` — override test-runner-а для тестів цього модуля (`runFixPipeline`/`fixConcern`/`runRung`).
