...
**Часткове вимикання concern-ів:** `filterByDisabledConcerns` (після `filterByCapabilities`) відкидає concern-и, вимкнені записом `rule/concern` у `disable-rules` (весь rule лишається активним — фільтрується лише конкретний concern). Невідомий `rule/concern` id (немає такого concern-а серед відомих) — гучна помилка з "did you mean" на найближчий за префіксом concern того ж rule, а не тихий no-op.
