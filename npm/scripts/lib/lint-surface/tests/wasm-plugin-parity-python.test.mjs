 ...
 * реальний `plugin_lang_python.wasm` проти спільної для всіх гостей стелі
...
describe('wasm-plugin — size-budget (python/wasm-concerns, перша хвиля)', () => {
  test(`plugin_lang_python.wasm не перевищує бюджет ${WASM_SIZE_BUDGET_LABEL}`, async () => {
...
// =====================================================================
// `python/vscode_extensions` — policy-концерн (rego + snippet, без
...
const VSCODE_EXT_CONCERN_KEY = 'python/vscode_extensions'
const VSCODE_EXT_TARGET = '.vscode/extensions.json'
const VSCODE_EXT_CONCERN_DIR = join(REPO_ROOT, 'plugins', 'lang-python', 'rules', 'python', 'vscode_extensions')
...
async function runVscodeExtBoth(dir) {
  ...
  const jsResult = await evaluatePolicyConcern(
    { cwd: dir, ruleId: 'python', concernId: 'vscode_extensions' },
    {
      engine: 'rego',
...
describe('wasm-plugin parity — python/vscode_extensions (rego-канон через conftest vs wasm plugin-lang-python)', () => {
