...
Модуль парсить метадані скіла з файлу `main.json`, який є єдиним джерелом правди для визначення логіки його виконання. Обробляє такі параметри: умова автоактивації (`auto`), необхідність ізоляції у `worktree` та чи вимагає скіл запуску з кореня репозиторію (`requireRoot`). Це дозволяє іншим компонентам (як-от `auto-skills.mjs` та `n-cursor.js`) коректно інтерпретувати вимоги скілу та керувати його виконанням.
...
Since I don't have the code file to document, I will assume the request is to *reproduce the documentation style* based on the provided list and principles, which is essentially a request to apply the "behavior" description for generating documentation.
