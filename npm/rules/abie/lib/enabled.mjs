/**
 * Rule-level applies-гейт abie: чи `.n-rules.json:rules` (fallback — legacy `.n-cursor.json`) містить `abie`.
...
/**
 * Чи увімкнено правило **abie** у `.n-rules.json:rules`.
 * @param {string} root корінь репозиторію (cwd)
 * @returns {Promise<boolean>} `true` — `rules` містить `abie`; `false` — інакше
 */
export async function isAbieRuleEnabled(root) {
  let p = join(root, CONFIG_FILE)
  if (!existsSync(p)) p = join(root, LEGACY_CONFIG_FILE)
