...
describe('check test.location', () => {
  test('успіх: усі *.test.mjs у tests/ → exit 0', async () => {
    await withTmpCwd(async () => {
      await mkdir(join('rules/foo/fix/bar/tests'), { recursive: true })
      await writeFile(join('rules/foo/fix/bar/check.mjs'), 'export function check() {}\n')
      await writeFile(join('rules/foo/fix/bar/tests/check.test.mjs'), 'import { test } from "bun:test"\n')
  ...
  test('порушення: тест поряд із джерелом → exit 1', async () => {
    await withTmpCwd(async () => {
      await mkdir(join('rules/foo/fix/bar'), { recursive: true })
      await writeFile(join('rules/foo/fix/bar/check.mjs'), 'export function check() {}\n')
      await writeFile(join('rules/foo/fix/bar/check.test.mjs'), 'import { test } from "bun:test"\n')
