...
/**
 * Спільний system-стиль для всіх docgen-промптів: вимагає лаконічну поведінкову
...
export function sectionMessages(facts, src, anchors = null, intent = null, { complementaryBehavior = false } = {}) {
  ...
  let behaviorTask = 'нумерований алгоритм у бізнес-термінах'
  if (multi) {
    behaviorTask =
  ...
  if (complementaryBehavior) {
    behaviorTask = 'короткі поведінкові абзаци про відсутній користувацький контракт, без алгоритму'
  }
