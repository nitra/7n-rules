...
Початковий текст моделі очищується stripLeadingPreamble, щоб прибрати чатову самореференцію, а splitProtected та insertProtected зберігають захищену секцію «Призначення» під час усіх перетворень. Це важливо для повторних прогонів: наявний намір із попередньої документації не губиться, навіть якщо решта документа перегенеровується.

Оцінка якості через scoreDoc працює на зібраному Markdown, а не на сирому відповіді моделі: секції спочатку нормалізуються, потім порівнюються з фактами, захищеним блоком і дослівними якорями. Непідтверджена згадка файла або конфігурації також знижує score, тому відповідь не може пройти як придатна лише через формально правильні заголовки. Саме цей score визначає, чи результат придатний одразу, чи потрібен повторний прохід, і чи слід позначати документ degraded. buildApiSection додає в документ лише те, що справді випливає з уже відомого public API, тому не дублює очевидне й не вигадує відсутні деталі.

hasCompleteCommentDocumentation і commentDocumentationMode керують вибором маршруту з авторських коментарів. Якщо header і змістовні описи всіх public API повністю покривають документ, генерація завжди лишається детермінованою для JS, Rust і Python. Якщо коментарі часткові, запускається LLM fallback лише для відсутньої інформації; авторський текст не переписується.
...
- commentDocumentationMode — Вибирає маршрут за повнотою коментарів. Повний header і описи всіх
public API рендеряться дослівно без LLM; неповний контракт переходить у fallback.
