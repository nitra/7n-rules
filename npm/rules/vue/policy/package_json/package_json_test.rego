...
test_valid_vue_package if {
	count(deny) == 0 with input as valid_vue_package
}

test_non_vue_package_is_ignored if {
	count(deny) == 0 with input as {"devDependencies": {}}
}
...
test_rejects_esbuild_and_old_vite if {
	count(deny) == 2 with input as object.union(valid_vue_package, {
		"devDependencies": object.union(valid_vue_package.devDependencies, {
			"vite": "^7.0.0",
			"esbuild": "^0.25.0",
		}),
	})
