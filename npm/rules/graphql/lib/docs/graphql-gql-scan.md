...
### `extractVueScriptBlocks(sfc)` *(приватна)*
...
### `contentForGqlScan(content, filePath)` *(приватна)*
...
### `langFromPath(filePath)` *(приватна)*
...
### `virtualPathForParse(relativePath)` *(приватна)*
...
### `astContainsGqlTag(node)` *(приватна)*
...
### `sourceFileHasGqlTaggedTemplate(content, relativePath)` *(експорт)*
...
### `isGqlScanSourceFile(relativePath)` *(експорт)*
...
### `shouldSkipFileForGqlScan(relativePosix)` *(експорт)*
