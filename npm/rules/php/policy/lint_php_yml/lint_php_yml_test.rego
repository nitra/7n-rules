...
template_data := {"snippet": {"jobs": {"php": {"steps": [{"run": "n-cursor lint php --read-only"}]}}}}

test_allow_canonical if {
	pkg := {"jobs": {"php": {"steps": [{"run": "n-cursor lint php --read-only"}]}}}
...
test_deny_no_lint_php_run if {
	...
	contains(msg, "n-cursor lint php --read-only")
}
