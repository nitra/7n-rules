...
Файл потрібен як спільна логіка для `opa_check` і `regal`: він вирівнює вибір цілей для обох read-only регo-детекторів — або конкретні `.rego` з `ctx.files`, або весь policy-корінь `npm/rules`, коли явних файлів немає. `FULL_TARGET="npm/rules"` — константа для спільного кореня policy-дерева. Також тут зосереджені `resolveTargets` і `runStep`, щоб обидва `main.mjs` мали однаковий контракт поведінки під час запуску зовнішнього кроку та обробки його завершення.
