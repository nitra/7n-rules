...
describe("run-shellcheck error paths", () => {
  ...
  test("runShellcheckText returns 1 + prints patch hint when patch absent (lines 53, 110-111)", async () => {
    if (!resolveCmd("shellcheck")) { expect(true).toBe(true); return }
    const actual = await vi.importActual("node:child_process")
    vi.mocked(spawnSync)
  ...
  test("autofixOneFile: diffResult.error => stderr + return 1 (lines 145-146)", async () => {
    ...
    vi.mocked(spawnSync)
      ...
      .mockReturnValueOnce({ // shellcheck -f diff -> spawn error
  ...
  test("runFinalShellcheck: finalRun.error => stderr + return 1 (lines 209-210)", async () => {
    ...
    vi.mocked(spawnSync)
      ...
      .mockReturnValueOnce({ // final shellcheck call -> spawn error
  ...
  test("applyShellcheckDiff: patch fails => stderr + return 1 (lines 186-188)", async () => {
    ...
    vi.mocked(spawnSync)
      ...
      .mockReturnValueOnce({ status: 0, stdout: "true\n", stderr: "", error: null, pid: 0, signal: null }) // rev-parse
      .mockReturnValueOnce({ status: 0, stdout: "x.sh\0", stderr: "", error: null, pid: 0, signal: null }) // ls-files
      .mockReturnValueOnce({ // shellcheck -f diff -> non-empty diff (has autofixable issues)
        status: 1, stdout: "--- a/x.sh\n+++ b/x.sh\n@@ -1 +1 @@\n-echo $1\n+echo \"$1\"\n",
        stderr: "", error: null, pid: 0, signal: null
      })
      .mockReturnValueOnce({ // patch -p1 -> fails
        status: 1, stdout: "patch failed output\n", stderr: "patch error msg\n",
        error: null, pid: 0, signal: null
      })
    const errLines = []
    const origErr = process.stderr.write.bind(process.stderr)
    process.stderr.write = chunk => { errLines.push(String(chunk)); return true }
