/**
 * Тести native-фіксу `text/oxfmtrc` (§2.74, родина
 ...
 * `loadT0Patterns` віддає синтетичний native-патерн замість `fix-oxfmtrc.mjs`,
...
const ruleId = 'text'
const concernId = 'oxfmtrc'
const TARGET = '.oxfmtrc.json'
const SNIPPET = readFileSync(join(CONCERN_DIR, 'template', '.oxfmtrc.json.snippet.json'), 'utf8')
...
test('loadT0Patterns повертає синтетичний native-fix pattern, не fix-oxfmtrc.mjs', async () => {
...
test('домерджування у наявний конфіг: канон додано, локальне збережено', async () => {
  await withTmpDir(async dir => {
    ...
    expect(written.singleQuote).toEqual(true)
