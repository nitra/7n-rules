 ...
 *
 * `listShellScriptPaths`/`runShellcheckText` і їхні приватні залежності (`printShellcheckInstallHints`,
 * `printPatchInstallHints`, `autofixOneFile`, `shouldStopAutofixLoop`, `applyShellcheckDiff`,
 * `runFinalShellcheck`) перенесені сюди з видаленого `main.mjs` (read-only детектор-бік того
 * самого концерну портовано в `rules-core`, `crates/rules-core/src/concerns/text_run_shellcheck.rs`) —
 * write-режим (`readOnly=false`) лишається виключно JS-стороною, тому цей файл тепер самодостатній.
 * ShellCheck не має прапорця `--fix`; для виправлень, які інструмент уміє запропонувати, використовується
 * формат виводу `diff` і застосування патчу через `patch -p1` у корені проєкту (шляхи у unified diff від
 * ShellCheck узгоджуються з цим режимом).
 *
...
/**
 * Фінальний прогін `shellcheck` по всіх файлах — без `-f diff`, щоб отримати звичайний звіт.
