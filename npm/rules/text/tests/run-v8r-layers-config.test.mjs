 ...
 * `nitra/mt`). Перевіряє реальним v8r-прогоном через каталог `@7n/rules`, що валідний конфіг
...
describe('layers-config.json schema (v8r-catalog)', () => {
  ...
  test('валідний docs/layers.json проходить v8r без помилок', async () => {
    ...
    expect(await runV8rWithFiles([file])).toEqual({ code: 0, detail: '' })
  })
  ...
  test('docs/layers.json з невідомою властивістю падає (additionalProperties: false), detail пояснює причину', async () => {
    ...
    const { code, detail } = await runV8rWithFiles([file])
