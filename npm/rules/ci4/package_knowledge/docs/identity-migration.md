...
- `npm/rules/ci4/package_knowledge/tests/identity-migration.test.mjs` (reconcileTopicIdentities) — keeps topic ID, aliases and narrative mapping when an unchanged file moves; recognizes a symbol rename from semantic signature and graph neighborhood; blocks ambiguous splits and merges with an explicit migration plan; preserves a protected MANUAL/EXPECTED registry only through an unambiguous mapping; orders mappings and topics identically regardless of input ordering
