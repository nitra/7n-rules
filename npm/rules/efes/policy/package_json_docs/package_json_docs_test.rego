# Тести `efes.package_json_docs`. Запуск:
#   conftest verify -p npm/rules/efes/policy/package_json_docs
package efes.package_json_docs_test

import data.efes.package_json_docs
import rego.v1

test_allow_present if {
	pkg := {"devDependencies": {"@nitra/efes-docs": "^1.0.0"}}
	count(package_json_docs.deny) == 0 with input as pkg
}

test_allow_present_with_other_devdeps if {
	pkg := {"devDependencies": {"@nitra/efes-docs": "1.2.3", "vite": "^5.0.0"}}
	count(package_json_docs.deny) == 0 with input as pkg
}
...
test_deny_only_in_dependencies if {
	pkg := {"dependencies": {"@nitra/efes-docs": "^1.0.0"}}
	count(package_json_docs.deny) > 0 with input as pkg
}
