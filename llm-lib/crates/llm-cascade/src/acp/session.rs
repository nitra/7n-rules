...
impl PermissionRequestEvent {
    ...
    ///
    /// # Errors
    /// [`CascadeError::Provider`] — з'єднання з агентом уже закрите.
    pub fn respond(
        self,
    ...
    /// # Errors
    /// [`CascadeError::Provider`] — з'єднання з агентом уже закрите.
    pub fn cancel(self) -> Result<(), CascadeError> {
        self.responder
            .respond(RequestPermissionResponse::new(
...
enum SessionCommand {
    Prompt {
        text: String,
        reply: oneshot::Sender<Result<StopReason, CascadeError>>,
    },
    Cancel,
}
...
impl SessionHandle {
    ...
    ///
    /// # Errors
    /// [`CascadeError::Provider`] — фонова задача сесії вже завершилась
    /// (з'єднання розірване) або хід провалився ACP-помилкою/idle-timeout.
    pub async fn prompt(&self, text: impl Into<String>) -> Result<StopReason, CascadeError> {
        ...
        self.commands
            ...
            .map_err(|_| CascadeError::Provider("acp: сесія вже завершена".to_string()))?;
        reply_rx.await.map_err(|_| {
            CascadeError::Provider("acp: фонова задача сесії обірвала канал відповіді".to_string())
        })?
    }
    ...
    ///
    /// # Errors
    /// [`CascadeError::Provider`] — фонова задача сесії вже завершена.
    pub fn cancel(&self) -> Result<(), CascadeError> {
        self.commands
...
///
/// # Errors
/// [`CascadeError::Provider`] — спавн/handshake/config-крок провалились.
pub async fn create_session(
    spec: AcpAgent,
    cwd: &Path,
    options: SessionOptions,
) -> Result<(SessionHandle, mpsc::UnboundedReceiver<SessionEvent>), CascadeError> {
    ...
    match ready_rx.await {
        ...
        Ok(Err(message)) => Err(CascadeError::Provider(message)),
        Err(_) => Err(CascadeError::Provider(
...
) -> Result<StopReason, CascadeError>
where
