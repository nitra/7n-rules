...
impl LocalCloud {
    ...
    ///
    /// # Errors
    /// [`CascadeError::NoModelConfigured`] якщо для тиру не задано жодної
    /// env-змінної; [`CascadeError::Provider`] на помилку самого виклику.
    pub async fn one_shot(
        ...
        let spec = resolve_model(tier).ok_or(CascadeError::NoModelConfigured(tier))?;
        let (provider, model_name) =
            parse_model_spec(&spec).map_err(CascadeError::InvalidModelSpec)?;
        ...
        res.first_text()
            .map(str::to_owned)
            .ok_or_else(|| CascadeError::Provider("порожня відповідь моделі".to_string()))
    }
}
