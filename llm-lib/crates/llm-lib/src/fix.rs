...
pub struct FixRequest {
    ...
    /// Рецепт для МОДЕЛІ, коли повідомлення детектора адресоване людині.
    ///
    /// Повідомлення детектора — елемент UX: воно радить людині зручну
    /// команду («додай `npx @7n/n ch`»). Циклу така порада недосяжна —
    /// shell поза його набором інструментів свідомо, — і живий прогін
    /// показав ціну цієї розбіжності: єдиний наявний рецепт виявився
    /// забороненим, іншого не було, і модель вигоріла всю стелю ходів,
    /// повторюючи `ls` і не спробувавши жодного запису.
    ///
    /// Сюди кладуть те, що команда РОБИТЬ: який файл створити і якої він
    /// форми. `None` — повідомлення й так самодостатнє (більшість
    /// concern-ів).
    pub fix_hint: Option<String>,
    /// Температура генерації. `None` — не патчимо, діє дефолт провайдера.
    ///
    /// Виноситься в запит, бо правильне значення залежить від моделі рунга і
    /// вимірюється, а не виводиться. Спокуса поставити `0.0` («ремонт — не
    /// творчість, хай буде відтворювано») живим прогоном спростована:
    /// greedy позбавляє слабку модель єдиного способу вийти з повтору, і
    /// вона зробила 47 однакових викликів поспіль замість жодної правки,
    /// тоді як із ненульовою температурою та сама модель на тому самому
    /// вході писала.
    /// Нуль лишається корисним — але для ВИМІРЮВАННЯ (відтворювані прогони),
    /// а не як бойовий дефолт.
