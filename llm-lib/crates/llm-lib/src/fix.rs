...
pub enum StopReason {
    ...
    ProviderError,
    /// Багато викликів інструментів і жодного запису — задача не
    /// закривається наявним набором інструментів (типовий приклад:
    /// порушення усувається ВИДАЛЕННЯМ файлу, а `delete` у наборі немає).
    NoProgress,
}
...
pub struct FixRequest {
    ...
    /// Стеля output-токенів на один хід моделі. `None` — консервативний
    /// дефолт циклу. Виносимо в запит, бо потрібне значення залежить від
    /// моделі рунга: слабкій локальній стеля рятує від розгону у відповідь
    /// на кілька екранів, а великій контекстній вона ж обрізає легітимний
    /// багатофайловий фікс на півслові.
