...
/// Причина відмови для запису поза git-root — з рецептом, а не лише з
/// діагнозом.
///
/// Найчастіше сюди приводить не зловмисний шлях, а зіпсований: слабка модель
/// повторює абсолютний `cwd` з промпта і на довгому шляху ламає його токен за
/// токеном (живий прогін дав перекручений сегмент імені й цілий випущений
/// сегмент). Формально це справді запис поза деревом, і veto правильне — але
/// голий діагноз моделі не допомагає, і вона повторює ту саму помилку до
/// вигоряння стелі ходів.
///
/// Рецепт живе саме тут, а не в преамбулі, свідомо: у преамбулі загальна
/// вказівка конкурує за увагу з рештою завдання ще до того, як стане
/// потрібною, а тут вона приходить рівно в момент помилки й стосується
/// конкретного шляху.
fn out_of_root_reason(raw_path: &str) -> String {
    format!(
        "запис поза git-root: {raw_path} — вказуй шлях ВІДНОСНО робочої \
         директорії (наприклад `pkg/.changes/зміна.md`), не абсолютний"
    )
}
...
impl WriteGuard {
    ...
    pub fn check_write(&mut self, raw_path: &str) -> Decision {
        ...
        let Some(root) = self.root.clone() else {
            return self.block(&abs, out_of_root_reason(raw_path));
        };
