...
one_shot_acp та one_shot_local_cloud виконують окремі запити до LLM-провайдерів, де помилки валідації типів або невідповідності конфігурації повертаються як винятки. При використанні `resolve_model` результат може бути відсутнім, якщо вказаний тир не знайде відповідності в оточенні.
