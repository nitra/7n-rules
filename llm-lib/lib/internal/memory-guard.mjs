/** @see ./docs/memory-guard.md */

/**
 * Детекція memory-guard rejection локального model-сервера (нема RAM на prefill)
 * для llm-lib раннерів (one-shot / agent-skill / agent-fix). INTERNAL — не входить
 * у публічний API пакета.
 *
 * Ретраїти нема куди — RAM-стеля фіксована, повторний виклик згорить так само
 * (fail-fast політика пакета). Тому замість structured-error-повернення й
 * circuit-breaker'а: друк тіла запиту в stdout і Error, аби причина була видна одразу.
 */
